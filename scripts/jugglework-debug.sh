#!/usr/bin/env bash
#
# jugglework-debug.sh — one-stop observability + lifecycle control for the
...
#   kill-orphans    remove orphan jugglework/opencode processes (ppid == launchd)
...
#   wait-healthy    block until jugglework-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Variant (JUGGLEWORK_DEV_VARIANT):
#   electron   (default) pnpm --filter @jugglework/desktop dev:electron
...
#   - ~/Library/Application Support/com.juggleai.jugglework.dev/** (tokens,
...
DEV_LOG_FILE="${JUGGLEWORK_DEV_LOG_FILE:-$HOME/.jugglework/debug/jugglework-dev.log}"
PNPM_DEV_LOG="${JUGGLEWORK_PNPM_DEV_LOG:-/tmp/jugglework-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${JUGGLEWORK_PNPM_DEV_PID:-/tmp/jugglework-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${JUGGLEWORK_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${JUGGLEWORK_ELECTRON_REMOTE_DEBUG_PORT:-9823}"
...
DEV_VARIANT="${JUGGLEWORK_DEV_VARIANT:-electron}"
case "$DEV_VARIANT" in electron|tauri) ;; *)
  printf '[jugglework-debug] unknown JUGGLEWORK_DEV_VARIANT=%s (expected electron|tauri)\n' "$DEV_VARIANT" >&2
  exit 2
  ;;
esac
...
discover_jugglework_server_port() {
  ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/jugglework-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.juggleai\.jugglework/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.JUGGLEWORK_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  local port
  port=$(discover_jugglework_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev jugglework-server running)"
  else
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with JUGGLEWORK_DEV_LOG_FILE set)"
  fi
}
...
diagnose_hang() {
  ...
  echo "=== jugglework hang diagnosis ==="
  echo "time=$now"
  echo "repo=$REPO_ROOT"
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(JUGGLEWORK_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_jugglework_server_port)
  if [[ -n "$port" ]]; then
    echo "  jugglework-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no jugglework-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/jugglework-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set JUGGLEWORK_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "target/debug/JuggleWork-Dev"
...
start() {
  ...
  case "$DEV_VARIANT" in
    electron)
      ...
      env JUGGLEWORK_DEV_LOG_FILE="$DEV_LOG_FILE" \
        nohup pnpm --filter @jugglework/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
      pid=$!
      ;;
    tauri)
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "jugglework-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "jugglework-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the jugglework-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.juggleai.jugglework.dev"
