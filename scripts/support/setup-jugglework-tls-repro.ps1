param(
    [string]$Hostname = "poc.jugglework.test",
...
function Stop-ReproJobs {
    try {
        $jobs = Get-Job -Name "JuggleWorkTlsRepro-*" -ErrorAction SilentlyContinue
        foreach ($job in $jobs) {
...
function Invoke-Cleanup {
    ...
    if (-not $Quiet) {
        Write-Step "Cleaning JuggleWork TLS repro artifacts..."
    }
...
function Start-ReproListenerJob {
    ...
    $jobName = "JuggleWorkTlsRepro-{0}" -f $Port
    ...
    $job = Start-Job -Name $jobName -ArgumentList $Port, $Label -ScriptBlock {
        ...
        try {
            while ($true) {
                ...
                $context.Response.Headers.Add("X-JuggleWork-TLS-Repro", $Label)
...
Write-Step "JuggleWork TLS repro setup"
...
$rootSubject = "CN=JuggleWork TLS Repro Root CA, O=$Marker"
$healthyIntermediateSubject = "CN=JuggleWork TLS Repro Healthy Intermediate CA, O=$Marker"
$brokenIntermediateSubject = "CN=JuggleWork TLS Repro Broken Intermediate CA, O=$Marker"
...
Write-Step ("   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\jugglework-doctor.ps1 -WebUrl {0} -ApiUrl {1} -ExpectedIssuerMatch `"JuggleWork TLS Repro`"" -f $healthyUrl, $brokenUrl)
...
Write-Step "   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-jugglework-tls-repro.ps1 -Cleanup"
