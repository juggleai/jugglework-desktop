...
You are an agent helping a user install and set up JuggleWork.

Goal: install JuggleWork, create a provisional workspace without requiring email
...
Install the `jugglework-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/jugglework-bootstrap/bin/jugglework.mjs install \
  --install-dir "$HOME/.jugglework/bootstrap" \
...
`jugglework-bootstrap` CLI (a single dependency-free Node file) and installs it
into `$HOME/.local/bin` - no npm or npx required.

```bash
curl -fsSLo /tmp/jugglework-install.sh https://juggle.im/install.sh
less /tmp/jugglework-install.sh
sh /tmp/jugglework-install.sh
```
...
```bash
jugglework-bootstrap install app \
  --manifest https://juggle.im/install-manifest.json \
  --json
```
...
```bash
jugglework-bootstrap cloud bootstrap-workspace \
  --base-url https://work.juggle.im \
  --workspace-name "<workspace-name>" \
  --skill-name "First JuggleWork Skill" \
  --claim-roles owner \
  --prepare-desktop \
...
1. Confirm in one line that JuggleWork is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "JuggleWork
...
4. End with this single question: "Want me to connect the JuggleWork MCP to this
...
1. A provisional workspace must be claimed before its owner can authenticate
   the JuggleWork MCP. If it has not been claimed yet, explain that dependency and
...
3. For Codex, run:
   ...
   If an `jugglework` entry already exists, do not add a duplicate. Authenticate
   the existing entry instead. To switch organizations or recover stale auth,
   run `codex mcp logout jugglework` before `codex mcp login jugglework`.
4. For another agent, use its current instructions from
   `https://juggle.im/docs/cloud/run-in-the-cloud/cloud-mcp`; do not guess
...
5. Tell the user to restart or reopen the current agent after setup so the new
   ...
   client can see JuggleWork's `search_capabilities` and `execute_capability`
   tools.
...
```bash
jugglework-bootstrap cloud claim-link --role owner --json
```
...
- `jugglework-bootstrap doctor --json` returns `ok: true`
- `jugglework-bootstrap doctor --app --json` returns `ok: true`
- `jugglework-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
  - `organization.id` is present
  - `setup.id` is present
  - `skill.id` is present
  - `skillRun.triggered` is `true`
  - `skillRun.output` is `JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED`
  - `claimLinks[0].id` is present
  - `desktop.prepared` is `true`
  - `desktop.bootstrapPath` is present
  - `desktop.skillPath` is present
- `jugglework-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `jugglework-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/jugglework-bootstrap`).
- If app install fails: run `jugglework-bootstrap doctor --app --json` and report failed checks.
