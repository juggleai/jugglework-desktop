...
const cli = join(root, "bin", "jugglework.mjs")
const temp = mkdtempSync(join(tmpdir(), "jugglework-bootstrap-test-"))
...
try {
  ...
  const executableName = process.platform === "win32" ? "jugglework-bootstrap.cmd" : "jugglework-bootstrap"
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      claimLinks: [
        { id: "wcl_owner", role: "owner", token: "test-owner-token", url: "https://work.juggle.im/workspace-claim?token=test-owner-token", expiresAt: "2030-01-01T00:00:00.000Z" },
        { id: "wcl_member", role: "member", token: "test-member-token", url: "https://work.juggle.im/workspace-claim?token=test-member-token", expiresAt: "2030-01-01T00:00:00.000Z" },
      ],
    }),
    "utf8",
  )
  ...
  assert.equal(claimLinkByRoleJson.claimLinks[0].url, "https://work.juggle.im/workspace-claim?token=test-owner-token")
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "cob_test", title: "First JuggleWork Skill", output: "JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  // and `apiBaseUrl`, so a normal `--base-url https://work.juggle.im`
  // run broke the desktop app's Sign In flow (it opened
  // `https://work.juggle.im/?mode=sign-in...` and showed raw API JSON).
  await withStubDenApi(
    () => ({
      ok: true,
      organization: { id: "org_test", name: "Stub Org", slug: "org_test", status: "provisional" },
