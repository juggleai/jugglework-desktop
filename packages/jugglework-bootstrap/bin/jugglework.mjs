...
function printHelp() {
  console.log([
    "jugglework-bootstrap",
    "",
    "Usage:",
    "  jugglework-bootstrap install [--bin-dir <path>] [--install-dir <path>] [--source <path>] [--json]",
    "  jugglework-bootstrap install app --manifest <url-or-file> [--app-dir <path>] [--json]",
    "  jugglework-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  JUGGLEWORK_OWNER_PASSWORD=<password> jugglework-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  jugglework-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  jugglework-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
    "  install          Install the jugglework-bootstrap CLI into a user bin dir",
    ...
    "                   links). Defaults to https://work.juggle.im when",
    "                   --base-url is the hosted API (work.juggle.im);",
...
function defaultInstallDir() {
  return process.env.JUGGLEWORK_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".jugglework", "bootstrap")
}

function defaultBinDir() {
  return process.env.JUGGLEWORK_BIN_DIR || join(process.env.HOME || process.cwd(), ".local", "bin")
}

function defaultAppDir() {
  return process.env.JUGGLEWORK_APP_DIR || (process.platform === "darwin"
    ...
    : process.platform === "win32"
      ...
      : join(process.env.HOME || process.cwd(), ".local", "share", "jugglework"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.JUGGLEWORK_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "jugglework", "desktop-bootstrap.json")
}

function defaultSkillsDir() {
  return process.env.JUGGLEWORK_SKILLS_DIR || join(configHomeDir(), "opencode", "skills")
}

function defaultDeviceKeyPath() {
  return process.env.JUGGLEWORK_DEVICE_KEY_PATH || join(configHomeDir(), "jugglework", "bootstrap-device-key.json")
}
...
// browser opens `https://work.juggle.im/?mode=sign-in...` and shows raw
...
function deriveWebBaseUrl(apiBaseUrl) {
  try {
    ...
    if (url.hostname === "work.juggle.im") {
      return "https://work.juggle.im"
    }
    // Local/self-hosted deployments may proxy the API at a different port on
    // the same host. Callers that need this to be exact should pass
    // --web-base-url explicitly.
    return apiBaseUrl
  } catch {
    return apiBaseUrl
  }
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "jugglework-bootstrap-skill"
}
...
function runInstall(args) {
  ...
  const installedCli = join(installDir, "jugglework.mjs")
...
async function runInstallApp(args) {
  const json = hasFlag(args.flags, "json")
  const manifestLocation = getFlag(args.flags, "manifest") || process.env.JUGGLEWORK_INSTALL_MANIFEST
  ...
  const workDir = join(tmpdir(), `jugglework-app-install-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  try {
    ...
    writeFileSync(join(appDir, "jugglework-app-install.json"), JSON.stringify(install, null, 2))
...
async function runDoctor(args) {
  ...
  checks.push({ name: "juggleworkExecutable", ok: executableOk, value: executable })
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    ...
    checks.push({ name: "juggleworkApp", ok: existsSync(appPath), value: appPath })
...
async function createCloudSkillPlugin(baseUrl, auth, input) {
  ...
  const marketplace = marketplaces.body.items.find((item) => item?.name === DEFAULT_JUGGLEWORK_MARKETPLACE_NAME) || marketplaces.body.items[0]
  if (!marketplace?.id) {
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "juggleworkBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "juggleworkBootstrapOutput")
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "jugglework-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function resolveOwnerPassword(flags) {
  ...
  const envName = getFlag(flags, "owner-password-env", "JUGGLEWORK_OWNER_PASSWORD")
...
async function runCloudOnboard(args) {
  ...
  const skillOutput = getFlag(args.flags, "skill-output", "JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED")
...
async function runCloudBootstrapWorkspace(args) {
  const json = hasFlag(args.flags, "json")
  const baseUrl = getFlag(args.flags, "base-url", "https://work.juggle.im")?.replace(/\/$/, "")
  ...
  const skill = {
    ...response.body.skill,
    skillText: skillText(response.body.skill.title, response.body.skill.output || "JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED"),
  }

  const skillRun = runBootstrapSkill(skill, { trigger: "bootstrap.verify" })
  if (!skillRun.triggered || skillRun.output !== "JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED") {
    throw new Error(`skill_trigger_failed: ${JSON.stringify(skillRun)}`)
  }
  ...
  jsonOut({
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "jugglework-bootstrap cloud claim-link --role ${link.role}" to view`
