// Live local e2e fraimz for `jugglework install app` using a real macOS DMG.
//
...
// bin dir, then runs the installed `jugglework install app` command against the
// manifest and verifies it with `jugglework doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("jugglework install app DMG fraimz requires macOS")
}

const here = dirname(fileURLToPath(import.meta.url))
const packageRoot = resolve(here, "..")
const repoRoot = resolve(packageRoot, "..", "..")
const cli = join(packageRoot, "bin", "jugglework.mjs")
const outDir = join(repoRoot, "evals", "results", "jugglework-app-install-dmg")
const temp = join(tmpdir(), `jugglework-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
...
const manifestPath = join(temp, "jugglework-install-manifest.json")
const installedJuggleWork = join(binDir, "jugglework-bootstrap")
...
<key>CFBundleIdentifier</key><string>com.jugglework.fixture</string>
...
`)
  ...
  prove("The bootstrap CLI can be installed from a script", {
    action: "node bin/jugglework.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and an jugglework executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installedJuggleWork))

  const installApp = run(installedJuggleWork, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  ...
  prove("The installed CLI can download, verify, mount, and install JuggleWork.app from a DMG", {
    action: "jugglework install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
  ...
  const doctor = run(installedJuggleWork, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "jugglework doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with juggleworkApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "juggleworkApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "jugglework-app-install-dmg",
