// Live e2e fraimz for the script-installable JuggleWork bootstrap CLI.
//
...
//   DEN_API_E2E_BASE_URL=... node evals/agent-fraimz-jugglework-bootstrap.mjs
...
const temp = mkdtempSync(join(tmpdir(), "jugglework-bootstrap-e2e-"))
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const installedJuggleWork = join(binDir, process.platform === "win32" ? "jugglework-bootstrap.cmd" : "jugglework-bootstrap")
const outDir = join(repoRoot, "evals", "results", "jugglework-bootstrap-cli")
...
try {
  ...
  prove("A bootstrap script can install the jugglework CLI", {
  ...
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installedJuggleWork)

  const doctor = run(installedJuggleWork, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "jugglework doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installedJuggleWork, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
    ...
    skillName,
    "--json",
  ], { env: { ...process.env, JUGGLEWORK_OWNER_PASSWORD: randomPassword() }, timeout: 30_000 })
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "jugglework cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
  ...
  }, onboard.status === 0 && onboard.json?.ok === true && onboard.json?.organization?.id && onboard.json?.invitation?.invitationId && onboard.json?.skill?.id && onboard.json?.skill?.title === skillName && onboard.json?.skillRun?.triggered === true && onboard.json?.skillRun?.output === "JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED")
} finally {
...
<title>JuggleWork Bootstrap CLI — live e2e fraimz</title>
...
<h1>JuggleWork Bootstrap CLI — live e2e fraimz</h1>
