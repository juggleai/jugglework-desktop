...
// - `jugglework install app` installs a Linux tar.gz app artifact from a manifest,
// - `jugglework doctor --app` verifies the installed app,
// - `jugglework cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:<den-api-port>
//
// Run from packages/jugglework-bootstrap:
...
const outDir = join(repoRoot, "evals", "results", "jugglework-linux-container")
...
const artifactPath = join(fixtureDir, "jugglework-linux.tar.gz")
...
function redactSecrets(value) {
  return value.replace(/JUGGLEWORK_OWNER_PASSWORD=[^\s]+/g, "JUGGLEWORK_OWNER_PASSWORD=[redacted]")
}
...
try {
  const appBinary = join(artifactRoot, "jugglework")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho JuggleWork Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "jugglework"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/jugglework-linux.tar.gz", sha256: digest, appName: "jugglework" },
        arm64: { type: "tar.gz", url: "file:///fixture/jugglework-linux.tar.gz", sha256: digest, appName: "jugglework" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local jugglework tar.gz artifact and manifest with SHA-256",
  ...
  const installAndDoctor = dockerRun([
    "set -euo pipefail",
    "node /package/bin/jugglework.mjs install --install-dir /tmp/jugglework/install --bin-dir /tmp/jugglework/bin --json >/tmp/install.json",
    "/tmp/jugglework/bin/jugglework-bootstrap install app --manifest /fixture/install-manifest.json --app-dir /tmp/jugglework/apps --json >/tmp/app-install.json",
    "/tmp/jugglework/bin/jugglework-bootstrap doctor --install-dir /tmp/jugglework/install --bin-dir /tmp/jugglework/bin --app --app-dir /tmp/jugglework/apps --json",
  ].join(" && "), { timeout: 60_000 })
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run jugglework install, jugglework install app, and jugglework doctor --app inside Linux",
  ...
  const onboard = dockerRun([
    "set -euo pipefail",
    "node /package/bin/jugglework.mjs install --install-dir /tmp/jugglework/install --bin-dir /tmp/jugglework/bin --json >/tmp/install.json",
    `/tmp/jugglework/bin/jugglework-bootstrap cloud onboard --base-url "$DEN_API_E2E_BASE_URL" --owner-email ${JSON.stringify(ownerEmail)} --org-name ${JSON.stringify(`Linux CLI Org ${runId}`)} --invite-email ${JSON.stringify(inviteEmail)} --skill-name ${JSON.stringify(skillName)} --prepare-desktop --desktop-bootstrap-path /tmp/jugglework/desktop-bootstrap.json --skills-dir /tmp/jugglework/skills --json >/tmp/onboard.json`,
    "/tmp/jugglework/bin/jugglework-bootstrap doctor --install-dir /tmp/jugglework/install --bin-dir /tmp/jugglework/bin --desktop-bootstrap --desktop-bootstrap-path /tmp/jugglework/desktop-bootstrap.json --json >/tmp/doctor.json",
    ...
    env: ["-e", `JUGGLEWORK_OWNER_PASSWORD=${randomPassword()}`],
    timeout: 90_000,
  })
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run jugglework cloud onboard from inside Linux against live Den API",
    ...
    evidence: onboard,
  }, onboard.status === 0 && onboard.json?.onboard?.ok === true && onboard.json?.onboard?.organization?.id && onboard.json?.onboard?.invitation?.invitationId && onboard.json?.onboard?.skill?.id && onboard.json?.onboard?.skill?.title === skillName && onboard.json?.onboard?.skillRun?.triggered === true && onboard.json?.onboard?.skillRun?.output === "JUGGLEWORK_BOOTSTRAP_SKILL_TRIGGERED" && onboard.json?.onboard?.desktop?.prepared === true && onboard.json?.onboard?.desktop?.bootstrapPath && onboard.json?.onboard?.desktop?.skillPath && onboard.json?.doctor?.ok === true && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrap" && check.ok === true) && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrapHandoff" && check.ok === true))
} finally {
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "jugglework-linux-container",
