---
title: "JuggleWork MCP for Cursor"
sidebarTitle: "Cursor"
description: "Cursor MCP setup for JuggleWork Connect on Cursor Desktop, Cursor Web, and Cursor Agents."
---

The JuggleWork MCP server ([JuggleWork Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Cursor to use the JuggleWork capabilities your
...
Add JuggleWork as a remote MCP server in Cursor's MCP settings, paste the server
URL, and start OAuth from Cursor's connection prompt. Sign in to JuggleWork and
pick the organization Cursor should use.

Cursor Desktop's OAuth callback is the private-use URI
`cursor://anysphere.cursor-mcp/oauth/callback`. JuggleWork accepts exactly this
...
Other private-use callback schemes remain rejected: JuggleWork otherwise accepts
...
JuggleWork, and pick the organization Cursor should use.

Cursor Web and Cursor Agents use HTTPS OAuth callbacks, which JuggleWork
accepts.

## Switch organizations

Remove the stored MCP auth token for `jugglework`, then connect again and choose
...
steps match JuggleWork Connect's OAuth profile: HTTPS or HTTP loopback callbacks
