---
title: "JuggleWork MCP for Gemini CLI"
sidebarTitle: "Gemini CLI"
description: "Gemini CLI MCP setup for JuggleWork Connect, with OAuth access to your org's Gmail, Calendar, Drive, shared connections, skills, and Cloud resources."
---

The JuggleWork MCP server ([JuggleWork Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Gemini CLI to use the JuggleWork capabilities your organization has made
...
1. Add JuggleWork Connect as a remote HTTP MCP server:
   ...
   ```sh
   gemini mcp add --transport http jugglework https://work.juggle.im/jwork/api/mcp/agent
   ```
...
3. Authenticate JuggleWork inside Gemini:
...
Remove the stored MCP auth token for `jugglework`, then run `/mcp auth jugglework`
