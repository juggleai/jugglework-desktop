...
Organization branding lets an owner customize the JuggleWork experience for everyone signed in to an on-prem Den organization. Uploaded images stay inside the deployment: Den stores the binary image data in the existing MySQL database and serves signed, versioned URLs from your Den API origin.
...
<Note>
  Branding changes the displayed organization experience. It does not rename or re-sign the installed executable, and the signed application identity remains JuggleWork.
</Note>
...
config:
  public:
    webOrigin: "https://jugglework.example.internal"
    apiOrigin: "https://api.jugglework.example.internal"
    desktopDenBaseUrl: "https://jugglework.example.internal"
    corsOrigins: "https://jugglework.example.internal,https://api.jugglework.example.internal"
    betterAuthTrustedOrigins: "https://jugglework.example.internal"
    webAppHosts: "jugglework.example.internal"
```
...
If your organization uses a private certificate authority, install that CA in the operating-system trust store on every desktop. JuggleWork uses Electron networking for the native icon download, so it follows the operating system's proxy and certificate trust.
...
- Use `Clear` and save to return to stock JuggleWork branding.
...
2. Reopen JuggleWork to force an immediate configuration fetch instead of waiting for the hourly background refresh.
