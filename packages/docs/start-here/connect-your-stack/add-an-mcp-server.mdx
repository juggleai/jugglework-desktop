...
description: "Connect a custom or local MCP server that is not available through JuggleWork Connect"
---

Use this guide only when you need a custom or local MCP server that is not available through JuggleWork Connect.
...
Connecting a custom MCP server with JuggleWork takes four clicks.
...
back to JuggleWork. If a custom MCP needs behavior JuggleWork does not support,
[open an issue](https://github.com/juggleai/jugglework-desktop/issues/).
