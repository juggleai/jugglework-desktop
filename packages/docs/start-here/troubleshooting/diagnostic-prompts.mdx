...
- A connected service such as ServiceNow appears ready in JuggleWork Cloud, but the task does not expose `openwork-cloud_search_capabilities` or `openwork-cloud_execute_capability`.

**What this diagnostic checks**

The prompt below runs JuggleWork's bundled OpenCode executable against its generated runtime config and checks whether a fresh OpenCode process can connect to `openwork-cloud`. It does not attach to the OpenCode server already running inside JuggleWork, so it combines that result with the live status shown in JuggleWork and a forced tool call from a new task.
...
5. Find JuggleWork's generated `runtime-opencode-config.json` beside the active `server.json`. Respect discovered `OPENWORK_SERVER_CONFIG`, `OPENWORK_RUNTIME_DB`, or XDG overrides. Typical locations are:
