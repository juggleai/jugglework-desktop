export const JuggleWorkConnectInstaller = () => {
...
opencode mcp auth jugglework`;
  const installs = [
    ...
    },
    {
      ...
      supportStatus: "Setup only",
      supportExplanation: "Setup guide only: add JuggleWork, run codex mcp login jugglework, and reconnect with logout then login. Native proof must be rerun on this exact branch.",
      copyText: `codex mcp add jugglework --url ${MCP_SERVER_URL}`,
    ...
    },
    {
      ...
      copyText: `claude mcp add --transport http jugglework ${MCP_SERVER_URL}`,
    },
    {
      id: "opencode",
      label: "OpenCode",
  ...
  "mcp": {
    ...
    },
    {
      ...
      copyText: `code --add-mcp '{"name":"jugglework","type":"http","url":"${MCP_SERVER_URL}"}'`,
    },
    {
      id: "any-client",
      label: "Any client",
    ...
    <div id="connect-mcp-install" className="not-prose my-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-950">
      ...
      <div id={`connect-mcp-panel-${activeInstall.id}`} role="tabpanel" aria-labelledby={`connect-mcp-install-${activeInstall.id}`} className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="m-0 text-xs text-gray-500">Works with your JuggleWork account — <a href="https://work.juggle.im?mode=sign-up" className="font-medium underline">create one free</a>.</p>
