...
Use this when OpenCode runs on a remote machine, SSH host, or cloud VM, but the OAuth browser opens on your laptop or another local machine. In the commands below, `<server-name>` is the JuggleWork MCP name in your OpenCode config, often `openwork`.
