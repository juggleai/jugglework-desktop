...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in JuggleWork Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in JuggleWork Cloud

1. Open `MCP Connections` in the JuggleWork Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in JuggleWork Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted JuggleWork Cloud instance, the dashboard is
`https://work.juggle.im` and its public Den API origin is
`https://work.juggle.im`. Register this exact redirect URL with the OAuth
provider:

```text
https://work.juggle.im/v1/mcp-connections/oauth/callback
```

For a self-hosted instance whose public Den API is
`https://api.jugglework.example.com`, register:

```text
https://api.jugglework.example.com/v1/mcp-connections/oauth/callback
```
...
3. In JuggleWork Cloud, add the Slack connection and paste the app's client ID
   and client secret.
...
1. Sign in to JuggleWork, then open `Settings` > `Connect`.
...
4. The browser shows `Connected` — return to JuggleWork.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in JuggleWork Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through JuggleWork Connect,
...
services managed through JuggleWork Cloud.
