...
function formatToQueryStr(obj) {
  return Object.entries(obj).filter(([_, value]) => value !== undefined) // 过滤 undefined
  .map(([key, value]) => {
...
let ErrorMessages = [{
  ...
  code: 25016,
  msg: '阅读后销毁消息时间长度必须大于 0 ms',
  name: 'MESSAGE_LIFETIME_AFTER_READ_ERROR'
}, {
  code: 25017,
  msg: '定时删除消息时间长度必须大于 0 ms',
  name: 'MESSAGE_LIFETIME_ERROR'
}, {
  code: 21200,
  msg: '消息撤回成功',
  ...
  code: 21300,
  msg: '发布朋友圈参数不正确，content 和 mediaList 必须二选一或都传如',
  name: 'MOMENT_ADD_PARAM_ERROR'
}, {
  code: 21301,
  msg: '发布朋友圈参数不正确，mediaList 必须是数组',
  name: 'MOMENT_ADD_PARAM_MDL_ERROR'
}, {
  code: 0,
  msg: '内部业务调用成功',
...
function WebRequest () {
  ...
  let request = (url, option, callback) => {
    ...
    xhr.upload.onprogress = function (event) {
      ...
      let bodySize = body && typeof body.size === 'number' ? body.size : 0;
      let total = event.total || bodySize;
      if (!total) return;
      callback.progress({
        loaded: event.loaded,
        total,
...
async function signatureWithNonce(nonce, timestamp, signKey) {
  ...
  let cryptoKey = await crypto.subtle.importKey('raw', keyData, {
    name: 'HMAC',
    hash: 'SHA-256'
  }, false, ['sign']);
...
async function getPublishBody ({
  ...
  if (utils.isInclude([COMMAND_TOPICS.SEND_GROUP, COMMAND_TOPICS.SEND_PRIVATE, COMMAND_TOPICS.SEND_CHATROOM], topic)) {
    ...
    if (!utils.isEmpty(referMsg)) {
      ...
      // };
      var referBuffer = JTextEncoder$1.encoder(referContent);
    ...
    var bufferContent = JTextEncoder$1.encoder(content);
...
async function getQueryBody({
  ...
  if (utils.isEqual(COMMAND_TOPICS.UPDATE_MESSAGE, topic)) {
    ...
    content = utils.toJSON(content);
    let modifyBuffer = JTextEncoder$1.encoder(content);
  ...
  if (utils.isEqual(COMMAND_TOPICS.MSG_CONVERSATION_SEARCH, topic)) {
    ...
    let codec = $root.lookup('codec.SearchMsgsReq');
    let message = codec.create({
      keyword,
      count,
      channelType,
  ...
  if (utils.isInclude([COMMAND_TOPICS.GET_USER_STATUS, COMMAND_TOPICS.SUB_USER_STATUS], topic)) {
    ...
    let codec = $root.lookup('codec.UserIdsReq');
    let message = codec.create({
      userIds
    });
    targetId = userId;
...
async function msgFormat(msg, {
  ...
  if (msgContent && msgContent.length > 0) {
    ...
    // 内部信令消息忽略调用解密方法
    var fileIntenalMsgs = [MESSAGE_TYPE.RECALL, MESSAGE_TYPE.RECALL_INFO, MESSAGE_TYPE.READ_MSG, MESSAGE_TYPE.READ_GROUP_MSG,
    // MESSAGE_TYPE.MODIFY,
    MESSAGE_TYPE.CLEAR_MSG, MESSAGE_TYPE.CLEAR_UNREAD];
    ...
    content = JTextEncoder$1.decoder(content);
...
async function formatConversations$1(conversations, options = {}) {
  return await utils.Defer.all(conversations.map(async conversation => {
    let {
      msg,
      targetId,
      unreadCount,
    ...
    if (!utils.isEqual(msg.msgContent.length, 0)) {
      latestMessage = await msgFormat(msg, {
        currentUser,
        io
      });
    }
...
async function searchMsgsFormat(items, {
  currentUser,
  io
}) {
  items = await utils.Defer.all(utils.map(items, async item => {
    let {
      highlightText,
      msg,
      score
...
async function searchConversationFormat(result, {
  currentUser,
  io
}) {
  let {
  ...
  return {
    conversationId: targetId,
    conversationType: channelType,
    items: await searchMsgsFormat(items, {
      currentUser,
      io
    }),
    matchCount: maxScore
...
var tools$1 = {
  ...
  formatRTCRoom,
  formatStreams,
  searchConversationFormat
};

async function getQueryAckBody(stream, {
  cache,
  currentUser,
  io
}) {
  ...
  if (utils.isInclude([COMMAND_TOPICS.HISTORY_MESSAGES, COMMAND_TOPICS.SYNC_MESSAGES, COMMAND_TOPICS.GET_MSG_BY_IDS, COMMAND_TOPICS.GET_MERGE_MSGS], topic)) {
    result = await getMessagesHandler(index, data, {
      currentUser,
      io
    });
  }
  if (utils.isEqual(topic, COMMAND_TOPICS.SYNC_CHATROOM_MESSAGES)) {
    result = await getChatroomMsgsHandler(index, data, {
      currentUser,
      io
    });
  }
  ...
  if (utils.isInclude([COMMAND_TOPICS.CONVERSATIONS, COMMAND_TOPICS.PUBLIC_CONVERSATIONS, COMMAND_TOPICS.SYNC_CONVERSATIONS, COMMAND_TOPICS.QUERY_TOP_CONVERSATIONS], topic)) {
    result = await getConversationsHandler(index, data, {
      topic,
      currentUser,
      io
    });
  ...
  if (utils.isEqual(topic, COMMAND_TOPICS.GET_CONVERSATION)) {
    result = await getConversationHandler(index, data, {
      currentUser,
      io
    });
  }
  ...
  if (utils.isEqual(topic, COMMAND_TOPICS.GET_MENTION_MSGS)) {
    result = await getMentionMessages(index, data, {
      currentUser,
      io
    });
  }
  ...
  if (utils.isEqual(topic, COMMAND_TOPICS.GET_FIRST_UNREAD_MSG)) {
    result = await getMessage(index, data, {
      currentUser,
      io
    });
  }
  ...
  if (utils.isEqual(topic, COMMAND_TOPICS.GET_TOP_MSG)) {
    result = await getTopMessage(index, data, {
      currentUser,
      io
    });
  }
  if (utils.isEqual(topic, COMMAND_TOPICS.MSG_QRY_FAVORITE)) {
    result = await getFavtoriteMsgs(index, data, {
      currentUser,
      io
    });
  }
  ...
  if (utils.isEqual(topic, COMMAND_TOPICS.MSG_CONVERSATION_SEARCH)) {
    result = await getConversationSearch(index, data, {
      currentUser,
      io
    });
  }
  if (utils.isEqual(topic, COMMAND_TOPICS.MSG_GLOBAL_SEARCH)) {
    result = await getGlobalSearch(index, data, {
      currentUser,
      io
    });
  }
  if (utils.isEqual(topic, COMMAND_TOPICS.TAG_CREATE)) {
    result = await getCreateTagHandler(index, data, {
      currentUser,
      io
    });
  }
  if (utils.isInclude([COMMAND_TOPICS.GET_USER_STATUS, COMMAND_TOPICS.SUB_USER_STATUS], topic)) {
    result = await getUserStatus(index, data, {
      currentUser,
      io
    });
  }
...
async function getFavtoriteMsgs(index, data, {
  currentUser,
  io
}) {
  ...
  let list = await utils.Defer.all(utils.map(items, async item => {
    let {
      createdTime,
      msg
    } = item;
    let message = await tools$1.msgFormat(msg, {
      currentUser,
      io
    });
    return {
...
async function getTopMessage(index, data, {
  currentUser,
  io
}) {
  ...
  let message = {};
  if (msg) {
    message = await tools$1.msgFormat(result.msg, {
      currentUser,
      io
    });
...
async function getMentionMessages(index, data, {
  currentUser,
  io
}) {
  ...
  let msgs = await utils.Defer.all(utils.map(mentionMsgs, async msg => {
    return await tools$1.msgFormat(msg, {
      currentUser,
      io
    });
  }));
...
async function getConversationHandler(index, data, options = {}) {
  ...
  let conversations = await tools$1.formatConversations([item], options);
...
async function getConversationsHandler(index, data, options = {}) {
  ...
  conversations = await tools$1.formatConversations(conversations, options);
  return {
    conversations,
    isFinished,
    index
...
async function getCreateTagHandler(index, data, {
  currentUser,
  io
}) {
  let payload = $root.lookup('codec.UserConverTags');
...
async function getChatroomMsgsHandler(index, data, {
  currentUser,
  io
}) {
  ...
  let messages = await utils.Defer.all(utils.map(msgs, async msg => {
    return await tools$1.msgFormat(msg, {
      currentUser,
      io
    });
...
async function getMessage(index, data, {
  currentUser,
  io
}) {
  ...
  let msg = await tools$1.msgFormat(_msg, {
    currentUser,
    io
  });
  return {
...
async function getMessagesHandler(index, data, {
  currentUser,
  io
}) {
  ...
  let messages = await utils.Defer.all(utils.map(msgs, async msg => {
    ...
    let _msg = await tools$1.msgFormat(msg, {
      currentUser,
      io
    });
    return _msg;
...
async function getConversationSearch(index, data, {
  currentUser,
  io
}) {
  let payload = $root.lookup('codec.SearchMsgsResp');
  let result = payload.decode(data);
  let searchResult = await tools$1.searchConversationFormat(result, {
    currentUser,
    io
  });
  return {
...
async function getGlobalSearch(index, data, {
  currentUser,
  io
}) {
  let payload = $root.lookup('codec.QryGlobalSearchResp');
  let result = payload.decode(data);
  let {
    maxScore,
    items
  ...
  let list = await utils.Defer.all(utils.map(items, async item => {
    return await tools$1.searchConversationFormat(item, {
      currentUser,
      io
    });
  }));
...
async function getUserStatus(index, data, {
  currentUser,
  io
}) {
  let payload = $root.lookup('codec.UserStatusList');
  let result = payload.decode(data);
  let {
    items
  } = result;
...
async function getPublishMsgBody(stream, {
  currentUser,
  io
}) {
  let codec = $root.lookup('codec.PublishMsgBody');
  ...
  if (utils.isEqual(topic, COMMAND_TOPICS.NTF)) {
    ...
    let payload = $root.lookup('codec.DownMsg');
    let message = payload.decode(data);
    _msg = await tools$1.msgFormat(message, {
      currentUser,
      io
    });
    ...
    _msg = {
      ...
      ext: ext || '',
      attachedConversation: _attachedConver
    };
    ...
    switch (cmd) {
      ...
      case SIGNAL_CMD.PUBLISH:
        ...
        let {
          _msg,
          _name
        } = await getPublishMsgBody(stream, {
          currentUser,
          io
        });
        name = _name;
      ...
      case SIGNAL_CMD.QUERY_ACK:
        currentUser = io.getCurrentUser();
        result = await getQueryAckBody(stream, {
          cache,
          currentUser,
          io
        });
...
function TagSyncer(send, emitter, io, {
  ...
  let exec = ({
    ...
    let data = {
      topic: COMMAND_TOPICS.TAG_QUERY,
      userId
    };
...
function IO(config) {
  ...
  */
    ...
    async function smack({
      ...
      Network.detect(servers, (domain, error) => {
        ...
        if (error && utils.isEqual(403, error.status)) {
          updateState({
            state: CONNECT_STATE.DISCONNECTED,
            code: ErrorType.CONNECT_FORBIDDEN
          });
          return;
        }
        ...
        let {
          ws: protocol
        } = utils.getProtocol(currentDomain);
  ...
  let sendCommand = async (cmd, data, callback) => {
    ...
    let buffer = await encoder.encode(cmd, {
      callback,
      data,
      index,
      counter
  ...
  */
  ...
  let getPublicConversations = (params = {}) => {
    return utils.deferred((resolve, reject) => {
      ...
      let user = io.getCurrentUser();
      let _params = {
        topic: COMMAND_TOPICS.PUBLIC_CONVERSATIONS,
        time: 0,
        count,
        order,
        userId: user.id,
  ...
  */
  ...
  let createConversationTag = tag => {
    return utils.deferred((resolve, reject) => {
      ...
      let data = {
        topic: COMMAND_TOPICS.TAG_CREATE,
        userId,
        tag: _tags
      };
      io.sendCommand(SIGNAL_CMD.QUERY, data, result => {
        ...
        if (utils.isEqual(ErrorType.COMMAND_SUCCESS.code, code)) {
          ...
          utils.forEach(_tags, _tag => {
            let index = utils.find(tags, ackTag => {
              return utils.isEqual(_tag.id, ackTag.tag);
            });
            let isAdd = false;
            if (index > -1) {
...
let add = (messageId, content) => {
  ...
  let index = utils.find(contentList, ({
    seq
  }) => {
  ...
  if (utils.isEqual(index, -1)) {
    ...
    Storage.set(messageId, contentList);
  }
};
let get = messageId => {
  let contentList = Storage.get(messageId);
...
function Message$1 (io, emitter, logger) {
  ...
  io.on(SIGNAL_NAME.CMD_RECEIVED, (message, isPullFinished = true) => {
    ...
    if (utils.isEqual(message.name, MESSAGE_TYPE.COMMAND_STATUS_CHANGED)) {
      ...
      return emitter.emit(EVENT.USER_STATUS_CHANGED, {
        userId,
        status
      });
    }
    ...
    if (utils.isEqual(MESSAGE_TYPE.STREAM_TEXT, message.name)) {
      ...
      if (!is_finished) {
        streamStorge.add(messageId, {
          content,
          seq: 1
        });
      }
      message.content = utils.rename(message.content, {
  ...
  let sendMessage = (message, callbacks = {}) => {
    return utils.deferred((resolve, reject) => {
      ...
      if (utils.isNumber(lifeTime) && lifeTime <= 0) {
        return reject({
          error: ErrorType.MESSAGE_LIFETIME_ERROR
        });
      }
      if (utils.isNumber(lifeTimeAfterRead) && lifeTimeAfterRead <= 0) {
        return reject({
          error: ErrorType.MESSAGE_LIFETIME_AFTER_READ_ERROR
        });
      }
      let {
        referMsg
...
function Moment (io, emitter, logger) {
...
function Socket ($socket, {
  ...
  let funcs = ['connect', 'disconnect', 'getDevice', 'isConnected', 'isNeedConnect', 'getCurrentUser', 'setConnectParams', 'getUserStatus', 'subUserStatus'];
