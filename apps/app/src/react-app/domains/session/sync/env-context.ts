...
import { readJuggleWorkEnvPendingChanges } from "../../../../app/lib/jugglework-env-runtime";

const DEFAULT_CACHE_KEY = "__jugglework_env_default__";
const MAX_CONTEXT_CACHE_ENTRIES = 100;

const envSystemContextCache = new Map<string, string | undefined>();

export function clearJuggleWorkEnvSystemContextCache(): void {
...
export async function buildJuggleWorkEnvSystemContext(
  ...
  const readPendingChanges = options.readPendingChanges ??
    (() => readJuggleWorkEnvPendingChanges(options.runtimeKey));
  ...
  try {
    ...
    const context = [
      "JuggleWork environment variables configured:",
      keyList,
