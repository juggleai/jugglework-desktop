...
import { juggleworkServerRestart, type JuggleWorkServerInfo } from "../../../app/lib/desktop";
import {
...
type UseRemoteAccessRestartOptions = {
  isEnabled: () => boolean;
  onHostInfo: (info: JuggleWorkServerInfo) => void;
...
export function useRemoteAccessRestart(options: UseRemoteAccessRestartOptions) {
  ...
  const save = useCallback(
    async (enabled: boolean) => {
      ...
      const previous = readJuggleWorkServerSettings();
      ...
      try {
        const info = await juggleworkServerRestart({ remoteAccessEnabled: enabled }) as JuggleWorkServerInfo;
        writeJuggleWorkServerSettings({
        ...
        writeJuggleWorkServerSettings(previous);
        options.onSettingsChanged();
