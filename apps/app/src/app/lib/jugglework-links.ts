...
import { normalizeJuggleWorkServerUrl } from "./jugglework-server";
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("juggleworkHostUrl") ?? url.searchParams.get("juggleworkUrl") ?? "";
  const tokenRaw = url.searchParams.get("juggleworkToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeJuggleWorkServerUrl(hostUrlRaw);
  ...
  return {
    juggleworkHostUrl: normalizedHostUrl,
...
export function parseConnectDeepLink(rawUrl: string): ConnectDeepLink | null {
  ...
  const protocol = url.protocol.toLowerCase();
  if (protocol !== "jugglework:" && protocol !== "jugglework-dev:") {
    return null;
  }
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:jugglework-dev|jugglework|https?):\/\/[^\s"'<>]+/i);
