const PENDING_CHANGES_KEY = "jugglework.settings.environment.pendingChanges";
...
export function buildJuggleWorkEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "jugglework"}::${runtime || "runtime"}`;
}

export function readJuggleWorkEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeJuggleWorkEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
