...
type DevRoot = typeof globalThis & {
  __juggleworkDevLogSeq?: number;
  __juggleworkDevLogs?: DevLogRecord[];
};
...
export const recordDevLog = (
  ...
  const id = (root.__juggleworkDevLogSeq ?? 0) + 1;
  root.__juggleworkDevLogSeq = id;
...
export const clearDevLogs = () => {
  const root = globalThis as DevRoot;
  root.__juggleworkDevLogs = [];
  root.__juggleworkDevLogSeq = 0;
};
