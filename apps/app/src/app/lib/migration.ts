...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^jugglework\.react\.activeWorkspace$/,
  /^jugglework\.react\.sessionByWorkspace$/,
  /^jugglework\.server\.list$/,
  /^jugglework\.server\.active$/,
  /^jugglework\.server\.urlOverride$/,
  /^jugglework\.server\.token$/,
];
...
function electronMigrationBridge(): ElectronMigrationBridge | null {
  ...
  const bridge = (window as unknown as {
    __JUGGLEWORK_ELECTRON__?: { migration?: ElectronMigrationBridge };
...
export const MIGRATION_DEFER_KEY = "jugglework.migration.deferredUntil";
