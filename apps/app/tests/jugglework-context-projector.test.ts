...
import {
  buildJuggleWorkContext,
  screenFromRoute,
} from "../src/react-app/shell/jugglework-context-projector";
import type { WorkbenchSnapshot } from "../src/react-app/domains/session/chat/workbench-store";

type ContextProjectorInput = Parameters<typeof buildJuggleWorkContext>[0];
...
describe("JuggleWork context projector", () => {
  test("projects the focused split session and its panel state", () => {
    const context = buildJuggleWorkContext(baseInput);
  ...
  test("does not leak artifact tabs into a non-panel surface", () => {
    const context = buildJuggleWorkContext({
      ...baseInput,
      ui: {
        ...baseInput.ui,
