...
const LEGACY_USER_STATE_KEY = "jugglework.den.mcp.cloudControlUserState";
...
function health(input: { usable: boolean; failure?: JuggleWorkCloudMcpFailure | null }): JuggleWorkCloudMcpHealth {
  ...
  return {
    ...
    pluginCanaries: {
      expected: ["jugglework_docs_search"],
      present: usable ? ["jugglework_docs_search"] : [],
      missing: usable ? [] : ["jugglework_docs_search"],
    },
    compatibility: {
...
function configuredItem(): JuggleWorkMcpItem {
  return {
    name: CLOUD_MCP_SERVER_NAME,
    config: {
      type: "remote",
...
describe("cloud MCP maintenance user-state gate", () => {
  ...
  test("explicitly disabled entries stay skipped even without recorded intent", async () => {
    let mintCount = 0;
    const disabled: JuggleWorkMcpItem = { ...configuredItem(), config: { ...configuredItem().config, enabled: false } };
    const result = await syncCloudControlMcpInBackground({
      client: {
        baseUrl: scope.serverBaseUrl,
  ...
  test("reconciler: scoped 'removed' intent yields to configuredEnabled=true and blocks when the entry is absent", async () => {
    ...
    let mintCount = 0;
    const proceeding = await runJuggleWorkCloudMcpReconciler({
      mode: "repair",
      client: {
        baseUrl: scope.serverBaseUrl,
        getJuggleWorkCloudMcpHealth: async () => health({ usable: false }),
    ...
    const blocked = await runJuggleWorkCloudMcpReconciler({
      mode: "repair",
      client: {
        baseUrl: scope.serverBaseUrl,
        getJuggleWorkCloudMcpHealth: async () => health({ usable: false }),
