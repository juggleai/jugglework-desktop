...
description: 发送消息类 action。`args.name` 必须是 MessageType 字符串（如 `jg:video`），不能用 MIME（如 `text/plain`）；`args.content` 形状取决于 `name`，见下一节「message.name 取值 + message.content 形状」。
---
...
| args.content.file | File | 是 |  | 视频对象 | 1.0.0 |
