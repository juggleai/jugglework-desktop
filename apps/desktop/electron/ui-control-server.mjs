...
// and /command surface. Dispatched to the renderer's window.__juggleworkControl.
...
export function createUiControlServer({ appName, appIdentifier, getWindow }) {
  ...
  async function evaluateJuggleWorkControl(expression) {
  ...
  async function runJuggleWorkControlCommand(command, args = {}) {
    const argsJsonLiteral = jsonForJavaScript(args);
    if (command === "snapshot") {
      return evaluateJuggleWorkControl(`(async () => {
  ...
  async function start() {
    ...
    uiControlServer = createServer(async (request, response) => {
      try {
        ...
        if (request.method === "GET" && url.pathname === "/snapshot") {
          sendJsonResponse(response, 200, await runJuggleWorkControlCommand("snapshot"));
          return;
        }
        if (request.method === "GET" && url.pathname === "/actions") {
          sendJsonResponse(response, 200, await runJuggleWorkControlCommand("actions"));
          return;
        }
        if (request.method === "GET" && url.pathname === "/context") {
          sendJsonResponse(response, 200, await runJuggleWorkControlCommand("context"));
          return;
        }
        if (request.method === "POST" && url.pathname === "/query") {
          sendJsonResponse(response, 200, await runJuggleWorkControlCommand("query", await readJsonRequestBody(request)));
          return;
        }
        if (request.method === "POST" && url.pathname === "/command") {
          sendJsonResponse(response, 200, await runJuggleWorkControlCommand("command", await readJsonRequestBody(request)));
          return;
        }
        if (request.method === "POST" && url.pathname === "/execute") {
          sendJsonResponse(response, 200, await runJuggleWorkControlCommand("execute", await readJsonRequestBody(request)));
          return;
        }
    ...
    uiControlDiscoveryPath = path.join(app.getPath("userData"), "jugglework-ui-control.json");
    await writeFile(
    ...
    process.env.JUGGLEWORK_UI_CONTROL_DISCOVERY = uiControlDiscoveryPath;
  }
