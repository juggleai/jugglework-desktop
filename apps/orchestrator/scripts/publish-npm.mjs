...
if (!version) {
  throw new Error("jugglework-orchestrator version missing in apps/orchestrator/package.json")
}
...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `jugglework-${target.bun}${ext}`)
  if (!existsSync(src)) {
    throw new Error(`Missing jugglework binary at ${src}. Run: pnpm --filter jugglework-orchestrator build:bin:all`)
  }
  ...
  const dest = join(bindir, `jugglework${ext}`)
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for jugglework-orchestrator",
    license: "MIT",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      jugglework: `./bin/jugglework${ext}`,
      "jugglework-orchestrator": `./bin/jugglework${ext}`,
    },
    files: ["bin"],
  })
...
const meta = join(outroot, "jugglework-orchestrator")
mkdirSync(join(meta, "bin"), { recursive: true })

const wrapperSrc = join(root, "bin", "jugglework")
if (!existsSync(wrapperSrc)) {
  throw new Error(`Missing wrapper at ${wrapperSrc}`)
}
copyFileSync(wrapperSrc, join(meta, "bin", "jugglework"))
chmodSync(join(meta, "bin", "jugglework"), 0o755)
...
writeJson(join(meta, "package.json"), {
  name: "jugglework-orchestrator",
  version,
  description: "JuggleWork host orchestrator for opencode + JuggleWork server",
  license: "MIT",
  bin: {
    jugglework: "./bin/jugglework",
    "jugglework-orchestrator": "./bin/jugglework",
  },
  scripts: {
...
published.push({ name: "jugglework-orchestrator", dir: meta })
