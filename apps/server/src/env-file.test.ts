...
describe("env-file", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "jugglework-env-"));
  ...
  test("isReservedEnvKey blocks JUGGLEWORK_ / OPENCODE_ prefixes", () => {
    expect(isReservedEnvKey("JUGGLEWORK_TOKEN")).toBe(true);
  ...
  test("upsertMany rejects reserved keys", async () => {
    const svc = new EnvService({ path });
    const promise = svc.upsertMany([{ key: "JUGGLEWORK_TOKEN", value: "x" }]);
    await expect(promise).rejects.toBeInstanceOf(InvalidEnvKeyError);
  ...
  test("readForInjection strips reserved keys even if present on disk", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        updatedAt: Date.now(),
        variables: [
          { key: "JUGGLEWORK_TOKEN", value: "stolen" },
          { key: "ANTHROPIC_API_KEY", value: "sk-ant" },
        ],
      }),
    );
