...
async function makeWorkspace(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "jugglework-portable-files-"));
  tempDirs.push(dir);
...
describe("portable files", () => {
  test("lists only extra shareable .opencode files", async () => {
    ...
    await writeFile(join(workspaceRoot, ".opencode", "agents", "jugglework.md"), "# agent\n", "utf8");
    ...
    await writeFile(join(workspaceRoot, ".opencode", "jugglework.json"), '{"version":1}\n', "utf8");
    ...
    expect(files).toEqual([
      { path: ".opencode/agents/jugglework.md", content: "# agent\n" },
  ...
  test("rejects non-allowlisted portable files and path traversal", async () => {
    ...
    expect(() =>
      planPortableFiles(workspaceRoot, [{ path: ".opencode/jugglework.json", content: "{}" }]),
    ).toThrow(/not allowed/i);
