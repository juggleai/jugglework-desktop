...
const dirs: string[] = [];
const priorEnvStore = process.env.JUGGLEWORK_ENV_STORE;
const priorTokenStore = process.env.JUGGLEWORK_TOKEN_STORE;
const priorOpenAiApiKey = process.env.OPENAI_API_KEY;
const priorJuggleWorkApiKey = process.env.JUGGLEWORK_API_KEY;
const priorJuggleWorkInferenceBaseUrl = process.env.JUGGLEWORK_INFERENCE_BASE_URL;
...
beforeEach(() => {
  const dir = mkdtempSync(join(tmpdir(), "jugglework-env-routes-"));
  dirs.push(dir);
  // Redirect the shared env.json path into a throwaway dir so the test never
  // touches the developer's real ~/.config/jugglework/env.json.
  process.env.JUGGLEWORK_ENV_STORE = join(dir, "env.json");
  process.env.JUGGLEWORK_TOKEN_STORE = join(dir, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorEnvStore === undefined) {
    delete process.env.JUGGLEWORK_ENV_STORE;
  } else {
    process.env.JUGGLEWORK_ENV_STORE = priorEnvStore;
  }
  ...
  if (priorJuggleWorkApiKey === undefined) {
    delete process.env.JUGGLEWORK_API_KEY;
  } else {
    process.env.JUGGLEWORK_API_KEY = priorJuggleWorkApiKey;
  }
  if (priorJuggleWorkInferenceBaseUrl === undefined) {
    delete process.env.JUGGLEWORK_INFERENCE_BASE_URL;
  } else {
    process.env.JUGGLEWORK_INFERENCE_BASE_URL = priorJuggleWorkInferenceBaseUrl;
  }
...
describe("env routes", () => {
  ...
  test("invalid env store returns 409 instead of overwriting on PUT", async () => {
    writeFileSync(process.env.JUGGLEWORK_ENV_STORE!, "{ this is not json");
    const { base } = await boot();
  ...
  test("PUT rejects reserved keys with 400", async () => {
    ...
    const put = await fetch(`${base}/env`, {
      method: "PUT",
      headers: hostAuth(),
      body: JSON.stringify({ key: "JUGGLEWORK_TOKEN", value: "x" }),
    });
    expect(put.status).toBe(400);
    const body = (await put.json()) as { code: string; message: string };
    ...
    expect(body.message).not.toContain("JUGGLEWORK_TOKEN");
  });
  ...
  test("voice realtime session prefers JuggleWork Models broker when configured", async () => {
    ...
    const envPut = await fetch(`${base}/env`, {
      ...
      body: JSON.stringify({
        entries: [
          { key: "JUGGLEWORK_API_KEY", value: "ow_inf_test" },
          { key: "JUGGLEWORK_INFERENCE_BASE_URL", value: "https://inference.example.test" },
        ],
      }),
    });
  ...
  test("voice realtime session shows clear error when broker 503 and no local key", async () => {
    ...
    delete process.env.JUGGLEWORK_OPENAI_REALTIME_API_KEY;
    const { base } = await boot();
    ...
    expect(body.code).toBe("jugglework_models_voice_unavailable");
  ...
  test("voice realtime session does not fall back on non-503 broker errors", async () => {
    ...
    expect(body.code).toBe("jugglework_models_voice_failed");
  });
