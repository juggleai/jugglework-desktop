...
function createTestConfig(): ServerConfig {
  const tempDir = join(
    tmpdir(),
    `jugglework-google-workspace-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  return {
    host: "127.0.0.1",
    port: 8787,
...
const previousEnv = {
  devMode: process.env.JUGGLEWORK_DEV_MODE,
  plaintextVault: process.env.JUGGLEWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT,
  clientSecret: process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  legacyClientSecret: process.env.JUGGLEWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  brokerUrl: process.env.JUGGLEWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL,
};
...
afterEach(() => {
  restoreEnv("JUGGLEWORK_DEV_MODE", previousEnv.devMode);
  restoreEnv("JUGGLEWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT", previousEnv.plaintextVault);
  restoreEnv("GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.clientSecret);
  restoreEnv("JUGGLEWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.legacyClientSecret);
  restoreEnv("JUGGLEWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL", previousEnv.brokerUrl);
...
describe("Google Workspace extension", () => {
  test("reports only the user-configurable OAuth secret as missing", async () => {
    ...
    process.env.JUGGLEWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "";
    process.env.JUGGLEWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL = "";
  ...
  test("reads multi-account vaults and exposes active account", async () => {
    ...
    process.env.JUGGLEWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT = "1";
