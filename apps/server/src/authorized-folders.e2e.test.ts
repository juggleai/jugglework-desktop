...
async function createWorkspaceRoot() {
  return createTempRoot("jugglework-authorized-folders-");
}

async function startJuggleWorkServer(workspaceRoot: string, options?: { readOnly?: boolean }) {
...
beforeEach(async () => {
  const envRoot = await createTempRoot("jugglework-authorized-folders-env-");
...
describe("authorized folders routes", () => {
  test("lists visible folders and counts preserved hidden entries", async () => {
    ...
    const { base, config } = await startJuggleWorkServer(root);
  ...
  test("requires client auth, collaborator scope, and writable server", async () => {
    ...
    const readOnly = await startJuggleWorkServer(readOnlyRoot, { readOnly: true });
