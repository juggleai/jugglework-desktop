...
import {
  buildJuggleWorkRuntimeConfig,
  keepJuggleWorkRuntimeConfigFileFresh,
...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), "jugglework-runtime-config-file-"));
  roots.push(root);
  previousDb = process.env.JUGGLEWORK_RUNTIME_DB;
...
async function readConfigFile(config: ServerConfig): Promise<Record<string, unknown>> {
  const raw = await readFile(juggleworkRuntimeConfigFilePath(config), "utf8");
  return JSON.parse(raw) as Record<string, unknown>;
}

describe("jugglework runtime config file", () => {
  test("writes runtime-DB MCPs and jugglework defaults into the file", async () => {
    ...
    const path = await writeJuggleWorkRuntimeConfigFile(config, "ws_1");
    expect(path).toBe(juggleworkRuntimeConfigFilePath(config));
  ...
  test("jugglework prompt has a static search-first Memory Bank section, distinct from ## Memory", async () => {
    const { config } = await setup();
    await writeJuggleWorkRuntimeConfigFile(config, "ws_1");

    const parsed = await readConfigFile(config);
    const agent = parsed.agent as Record<string, { prompt?: string }>;
    const prompt = agent.jugglework?.prompt ?? "";
  ...
  test("keepJuggleWorkRuntimeConfigFileFresh rewrites the file on runtime-DB writes", async () => {
    const { config } = await setup();
    await writeJuggleWorkRuntimeConfigFile(config, "ws_1");
    cleanups.push(keepJuggleWorkRuntimeConfigFileFresh(config, "ws_1"));
  ...
  test("builds byte-stable config for repeated snapshots", async () => {
    ...
    const first = await buildJuggleWorkRuntimeConfig(config, "ws_1");
