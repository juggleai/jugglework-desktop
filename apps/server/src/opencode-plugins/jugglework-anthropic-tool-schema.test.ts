...
import { JuggleWorkAnthropicToolSchema } from "./jugglework-anthropic-tool-schema.js";
...
beforeAll(async () => {
  ...
  await JuggleWorkAnthropicToolSchema();
...
describe("JuggleWorkAnthropicToolSchema fetch patch", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./jugglework-anthropic-tool-schema.js");
    expect(Object.keys(mod)).toEqual(["JuggleWorkAnthropicToolSchema"]);
  });
});
