...
async function transformedSystem(plugin: Awaited<ReturnType<typeof JuggleWorkExtensionsPreview>>): Promise<string> {
...
function startFakeJuggleWorkServer() {
  ...
  process.env.JUGGLEWORK_SERVER_URL = `http://127.0.0.1:${server.port}`;
  process.env.JUGGLEWORK_SERVER_TOKEN = "test-token";
  return { requests };
}

describe("JuggleWorkExtensionsPreview session tools", () => {
  test("plugin entry exposes only the factory export for the OpenCode loader", () => {
    expect(Object.keys(JuggleWorkExtensionsPreviewEntry)).toEqual(["JuggleWorkExtensionsPreview"]);
  });
  ...
  test("projects built-in, extension, and Connect providers into one agent context", async () => {
        ...
        mcp: {
          status: async () => ({
            data: {
              ...
              "jugglework-cloud": { status: "connected" },
            },
          }),
        },
      },
    ...
    const output = await plugin.tool.jugglework_context.execute();
  ...
  test("routes semantic session queries without navigating the UI", async () => {
    ...
    const output = await plugin.tool.jugglework_query.execute({
      id: "session.read",
  ...
  test("uses the factory engine client as transform steering source of truth", async () => {
    ...
    const plugin = await JuggleWorkExtensionsPreview({ client: { mcp }, directory: "/tmp/archive" });
    const output: { system: string[] } = { system: [] };
  ...
  test("uses neutral transform steering when the engine reports failed Cloud status", async () => {
    const requests: unknown[] = [];
    const mcp = {
      result: { data: { "jugglework-cloud": { status: "failed" } } },
      async status(request: unknown) {
        requests.push(request);
        return this.result;
      },
    ...
    expect(requests).toEqual([{ query: { directory: "/tmp/archive" } }]);
    expect(output.system[0]).toBe(JUGGLEWORK_EXTENSION_DISCOVERY_INSTRUCTION);
    expect(output.system.join("\n")).toContain(JUGGLEWORK_LOCAL_SKILL_AUTHORING_INSTRUCTION);
    expect(output.system.join("\n")).not.toContain(JUGGLEWORK_CLOUD_SKILL_AUTHORING_INSTRUCTION);
  ...
  test("creates and starts multiple sessions through the JuggleWork backend", async () => {
    ...
    const output = await plugin.tool.jugglework_execute.execute({
      id: "session.create",
      args: {
        sessions: [
...
describe("JuggleWorkExtensionsPreview semantic tool surface", () => {
  test("exposes only the three semantic tools", async () => {
    ...
    expect(system).not.toContain("<jugglework_default_skill");
    expect(system).not.toContain("jugglework_ui_");
    expect(system).not.toContain("jugglework_session_");
    expect(system).not.toContain("jugglework_extension_");
    expect(system).not.toContain("jugglework_browser_");
