...
import { juggleworkOfficeAttachmentsPluginPath } from "../jugglework-extensions-plugin-path.js";
import { JuggleWorkOfficeAttachments } from "./jugglework-office-attachments.js";
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "jugglework-office-plugin-"));
  try {
    await fn(root);
  } finally {
...
async function transform(root: string, messages: unknown[]) {
  const plugin = await JuggleWorkOfficeAttachments({ directory: root });
...
describe("JuggleWorkOfficeAttachments", () => {
  ...
  test("rejects file URLs that resolve outside the workspace through a symlink", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "jugglework-office-outside-"));
      try {
        const docx = docxFixture();
  ...
  test("uses factory directory and ignores Daytona non-git worktree root", async () => {
    await withWorkspace(async (root) => {
      const docx = docxFixture();
      const plugin = await JuggleWorkOfficeAttachments({ directory: root, worktree: "/" });
      ...
      expect(materialized).toContain(".opencode/jugglework/inbox/chat-attachments/");
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    ...
    expect(plugin).toContain(juggleworkOfficeAttachmentsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("jugglework-office-attachments.ts");
  });

  test("module exposes only the plugin factory", async () => {
    const mod = await import("./jugglework-office-attachments.js");
    expect(Object.keys(mod)).toEqual(["JuggleWorkOfficeAttachments"]);
  });
});
