...
export function normalizeBlueprintSessionTemplates(jugglework: Record<string, unknown> | null | undefined): BlueprintSessionTemplate[] {
  const blueprint = readRecord(jugglework?.blueprint);
...
export function readMaterializedBlueprintSessions(jugglework: Record<string, unknown> | null | undefined): MaterializedBlueprintSession[] {
  const blueprint = readRecord(jugglework?.blueprint);
...
export function sanitizeJuggleWorkTemplateConfig(jugglework: Record<string, unknown> | null | undefined): Record<string, unknown> {
  const next = cloneRecord(jugglework ?? {});
...
export function applyMaterializedBlueprintSessions(
  jugglework: Record<string, unknown> | null | undefined,
  items: MaterializedBlueprintSession[],
  hydratedAt: number,
): Record<string, unknown> {
  const next = sanitizeJuggleWorkTemplateConfig(jugglework);
  const blueprint = readRecord(next.blueprint) ?? {};
  const materialized = readRecord(blueprint.materialized) ?? {};
