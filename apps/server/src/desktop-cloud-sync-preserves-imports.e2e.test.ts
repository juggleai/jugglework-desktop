...
async function createWorkspaceRoot() {
  const root = await mkdtemp(join(tmpdir(), "jugglework-desktop-cloud-sync-"));
  roots.push(root);
  return root;
}
...
describe("desktop cloud sync provider imports", () => {
  test("preserves workspace provider import baseline while recording sync state", async () => {
    ...
    const jugglework = expectRecord(configBody.jugglework, "workspace jugglework config");
    const cloudImports = expectRecord(jugglework.cloudImports, "workspace cloud imports");
