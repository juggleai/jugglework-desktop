 ...
 *
 * This is the single source of truth for the jugglework agent definition,
...
import {
  juggleworkExtensionsPreviewPluginPath,
  juggleworkCapabilitiesKnowledgePluginPath,
...
const JUGGLEWORK_AGENT_PROMPT = `You are JuggleWork.
...
export async function buildJuggleWorkRuntimeConfigObject(
  config?: ServerConfig,
  workspaceId?: string,
): Promise<Record<string, unknown>> {
  const runtimeConfig = config && workspaceId ? await readRuntimeOpencodeConfig(config, workspaceId) : {};
  return buildJuggleWorkRuntimeConfigObjectFromSnapshot(runtimeConfig);
}

export function buildJuggleWorkRuntimeConfigObjectFromSnapshot(
  ...
  return {
    ...
    agent: {
      jugglework: {
        ...
        mode: "primary",
        temperature: 0.2,
        prompt: JUGGLEWORK_AGENT_PROMPT,
        permission: {
          skill: {
...
export async function buildJuggleWorkRuntimeConfig(config?: ServerConfig, workspaceId?: string): Promise<string> {
  return stableStringify(await buildJuggleWorkRuntimeConfigObject(config, workspaceId));
}

export function juggleworkRuntimeConfigFilePath(config: ServerConfig): string {
 ...
 * mid-dispose.
 */
export async function writeJuggleWorkRuntimeConfigFile(config: ServerConfig, workspaceId: string): Promise<string> {
  const path = juggleworkRuntimeConfigFilePath(config);
  const job = async () => {
    const content = await buildJuggleWorkRuntimeConfig(config, workspaceId);
 ...
 */
export function keepJuggleWorkRuntimeConfigFileFresh(config: ServerConfig, workspaceId: string): () => void {
  return onRuntimeOpencodeConfigWrite((writeConfig, writtenWorkspaceId) => {
    if (writtenWorkspaceId !== workspaceId) return;
    void writeJuggleWorkRuntimeConfigFile(writeConfig, workspaceId).catch(() => undefined);
  });
}
