...
const gatedCallSchema = z.object({
  ok: z.literal(false),
  error: z.literal("use_jugglework_cloud"),
  message: z.string(),
}).passthrough();
...
const previousEnv = {
  runtimeDb: process.env.JUGGLEWORK_RUNTIME_DB,
  googleClientSecret: process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  legacyGoogleClientSecret: process.env.JUGGLEWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  tokenBrokerUrl: process.env.JUGGLEWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL,
...
function clearLegacyGoogleWorkspaceEnv() {
  ...
  delete process.env.JUGGLEWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET;
  delete process.env.JUGGLEWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL;
...
async function boot() {
  const root = await mkdtemp(join(tmpdir(), "jugglework-connect-gating-"));
  dirs.push(root);
...
afterEach(async () => {
  ...
  restoreEnv("JUGGLEWORK_RUNTIME_DB", previousEnv.runtimeDb);
  restoreEnv("GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.googleClientSecret);
  restoreEnv("JUGGLEWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.legacyGoogleClientSecret);
  restoreEnv("JUGGLEWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL", previousEnv.tokenBrokerUrl);
...
describe("Connect-aware legacy extension gating", () => {
  ...
  test("gates only non-status Google Workspace actions when Connect is enabled without legacy config", async () => {
    ...
    expect(actionKeys(actions)).toEqual([
      ...
      "media-generation/video_generate",
      "media-generation/video_job_cancel",
