...
function resolveJuggleWorkDataDir(): string {
  const override = process.env.JUGGLEWORK_DATA_DIR?.trim();
  if (override) return expandHome(override);
  return join(homedir(), ".jugglework", "jugglework-server");
}

export function auditLogPath(workspaceId: string): string {
  return join(resolveJuggleWorkDataDir(), "audit", `${workspaceId}.jsonl`);
}

export function legacyAuditLogPath(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "jugglework", "audit.jsonl");
}
