...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const envConfigPath = process.env.JUGGLEWORK_SERVER_CONFIG;
  const configPath = cli.configPath ?? envConfigPath ?? resolve(homedir(), ".config", "jugglework", "server.json");
  const fileConfig = await loadFileConfig(configPath);
  const configDir = dirname(configPath);

  const envWorkspaces = parseList(process.env.JUGGLEWORK_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.JUGGLEWORK_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.JUGGLEWORK_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.JUGGLEWORK_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.JUGGLEWORK_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.JUGGLEWORK_TOKEN;
  const hostTokenFromEnv = process.env.JUGGLEWORK_HOST_TOKEN;
  ...
  const approvalTimeoutMs =
    ...
    (process.env.JUGGLEWORK_APPROVAL_TIMEOUT_MS ? Number(process.env.JUGGLEWORK_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.JUGGLEWORK_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.JUGGLEWORK_READONLY;
  ...
  const envLogFormat = process.env.JUGGLEWORK_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.JUGGLEWORK_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.JUGGLEWORK_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.JUGGLEWORK_PORT ? Number(process.env.JUGGLEWORK_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
