...
Prefer synthetic paste for the JuggleWork composer:
...
- The target JuggleWork/Chrome window is visible and not covered by a native file
...
right window first with `wmctrl -a "JuggleWork - Dev"` or
`wmctrl -a "JuggleWork Cloud - Chromium"` so the screenshot shows the intended
journey step.
