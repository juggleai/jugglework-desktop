...
按 MCP 组件的实际云端或桌面端承载方式呈现插件可用性、操作与安装结果，避免把尚未就绪的能力误报为已启用。
...
插件详情弹窗 SHALL 先展示用户结果、影响与确定性主操作，再逐个列出 MCP 组件的名称、承载位置、就绪状态和组件操作：`cloud` 且待授权的行提供授权入口，`desktop` 且未安装的行提供安装到工作区入口。错误阶段、错误码、原始诊断和其他技术明细 MUST 默认折叠。详情 MUST NOT 用单一歧义按钮代表多个状态不同的组件。
...
#### Scenario: 组件操作失败并带技术诊断
- **WHEN** 插件详情收到含用户安全信息与技术诊断的结构化失败
- **THEN** 顶部先展示用户结果和重试操作，技术诊断保留在默认折叠区
...
### Requirement: 投递形态映射到规范生命周期
插件详情 SHALL 使用 `not_installed`、`installing`、`current`、`update_available`、`partial`、`needs_signin`、`needs_admin`、`failed`、`repair_required`、`removing` 作为唯一生命周期集合。纯 `cloud` 插件在全部组件可用时 SHALL 为 `current`，且不得要求工作区安装或卸载；纯 `desktop` 插件 SHALL 由当前工作区安装账本与解析版本决定；混合插件 SHALL 分别计算 Cloud 就绪度和桌面安装状态，并以未就绪结果聚合，不得把 Cloud 组件当作缺失本地文件。

#### Scenario: 纯云端插件已授权
- **WHEN** 纯 `cloud` 插件的所有组件均可用
- **THEN** 生命周期为 `current`，不展示安装到工作区或从工作区移除操作

#### Scenario: 混合插件仅缺桌面组件
- **WHEN** 混合插件的 Cloud 组件均可用，但桌面组件尚未安装
- **THEN** 生命周期为 `not_installed`，主操作仅安装桌面组件

#### Scenario: 混合插件需要成员登录
- **WHEN** 混合插件的桌面组件为当前版本，但 Cloud 组件需要成员登录
- **THEN** 生命周期为 `needs_signin`，主操作为登录，桌面组件保持 `current`

### Requirement: 详情状态与缓存服从实时身份
打开的插件详情 SHALL 通过当前组织和插件身份从实时 Marketplace 集合解析，不得保留卡片对象快照作为状态源。详情缓存 MUST 同时包含组织 ID、插件 ID 和解析版本。刷新失败 SHALL 只保留同一缓存键的最后正确数据并展示结构化失败；组织或版本变化 MUST 发起独立刷新，且不得短暂展示旧键结果。

#### Scenario: 详情打开期间解析版本变化
- **WHEN** 实时 Marketplace 集合中同一组织和插件出现新解析版本
- **THEN** 详情切换到新版本缓存键并重新计算为 `update_available`，旧版本结果不覆盖新状态

#### Scenario: 同键详情刷新失败
- **WHEN** 当前组织、插件和版本的详情刷新失败且存在最后正确数据
- **THEN** 用户仍可查看该数据，并看到刷新失败状态和重试入口，技术原因保持折叠
