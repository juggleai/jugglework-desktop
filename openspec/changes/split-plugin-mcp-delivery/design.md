...
**D8：投递聚合映射到统一生命周期。**
详情从实时组织/插件身份、解析版本、组件就绪度和本地安装账本投影 `not_installed`、`installing`、`current`、`update_available`、`partial`、`needs_signin`、`needs_admin`、`failed`、`repair_required`、`removing`。纯云端插件只受 Cloud 就绪度影响；纯桌面插件只受工作区安装影响；混合插件分别计算后按未就绪结果聚合。状态与主操作使用单一映射。

**D9：详情先展示结果，诊断信息按身份隔离。**
弹窗顶部展示用户可理解的结果、影响和主操作，随后展示组件承载与补齐动作；错误码、阶段、原始诊断和其他技术信息置于默认折叠区。详情按组织、插件、解析版本缓存；刷新失败只保留同键最后正确数据，并显示结构化失败和重试入口。
