...
- [ ] 27.3 Build exact committed `1.2.22` with the local release wrapper; notarize/staple the `.app` and outer DMG, regenerate final DMG blockmap/manifest, and pass complete packaged verification
- [ ] 27.4 Publish and fully verify all five immutable `v1.2.22/mac` objects on Qiniu without overwrite, then promote/read back Stable with the recorded `stable-1.2.22-only` pre-canary exception
- [ ] 27.5 Expose `1.2.22` through configuration-only Den metadata in China first and overseas second; update and deploy the website's Mac ARM64 download link
