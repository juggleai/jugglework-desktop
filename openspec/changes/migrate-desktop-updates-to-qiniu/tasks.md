...
- [ ] 21.5 Synchronize all three package versions to `1.2.20`, run focused package/runtime regression tests plus release-critical typechecks and Desktop tests, then commit/push dev and fast-forward main
...
- [ ] 21.7 Build and verify exact committed `1.2.20` macOS arm64 ZIP, DMG, blockmaps, packaged runtime contracts, Developer ID/Team, bundle id, hardened runtime, native inventory, and Qiniu-only updater configuration
- [ ] 21.8 Publish and fully verify all five immutable `v1.2.20/mac` objects without overwrite, then promote and read back Stable under its lock with controlled cache metadata using the two exact recovery exceptions
- [ ] 21.9 Expose `1.2.20` through configuration-only Den metadata in China first and overseas second; do not modify Windows, landing, organization policy beyond existing version inventory conventions, or production Den binaries
