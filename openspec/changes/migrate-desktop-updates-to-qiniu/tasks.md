...
- [x] 1.5 Migrate Alpha release/download constants and every updater-facing active GitHub URL, while preserving only the explicitly isolated legacy bridge publication tooling
...
- [x] 4.2 Implement dry-run output and strict version/channel/platform/architecture inputs with no filesystem, Qiniu, CDN, GitHub, or Den mutation
...
- [x] 4.7 Generate a non-secret release evidence artifact containing commit, version, keys, hashes, sizes, ETags, signing/notarization, manifest digest, CDN checks, bridge IDs, Den read-back, and timestamps
...
- [x] 8.1 Document routine Qiniu release, dry-run, verification-only, safe resume, promotion lock recovery, CDN cache refresh, Den ordering, bridge support, and higher-patch rollback
