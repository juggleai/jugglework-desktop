...
- [x] 28.3 Build and fully verify exact committed macOS ARM64 `1.2.23`, including Developer ID signing, app/DMG notarization, staples, Gatekeeper, final blockmaps, and runtime contracts
- [x] 28.4 Upload and fully verify all five immutable `v1.2.23/mac` objects, then promote the Stable object under the audited cache exception without cache expiry, refresh, or public Stable-manifest convergence validation
- [x] 28.5 Expose `1.2.23` through configuration-only Den metadata in China first and overseas second; update and deploy the website's Mac ARM64 download link

Release evidence: source commit `46d1bf476e2c8a7934bd230b181f23d1019f8252`; immutable manifest SHA-256 `a797114e3cd5e13467690a17c42f187dba2b4a002cbe0cb809a680f625825992`; ZIP SHA-256 `4133bada961900d8849769546b6682f4e06cc317fbd5feb242b20b13592ed58c`; DMG SHA-256 `b1f76da8441e896b8bb112a112884355f5bda612067e6f4c1f2ed5ca9a854b33`. Apple accepted separate `.app` submission `b390784c-010f-4d4b-9e1f-c291ac26db7c` and outer-DMG submission `337e635f-f1bb-4dbb-ace5-b64308ba4f71`; both staples, app/DMG Gatekeeper, disk-image verification, exact ARM64 inventory, packaged ZIP/DMG checks, native module checks, and UI MCP smoke test passed. The user explicitly authorized skipping only the local real-client upgrade canary. All immutable objects passed Qiniu metadata and public CDN MIME, length, byte-range, SHA-256, and SHA-512 verification. The user also explicitly authorized the `stable-1.2.23-only` cache exception: the Stable object write and Qiniu metadata verification passed under the promotion lock, but Cache-Control mutation, CDN refresh, and public Stable-manifest convergence were not performed. Public Stable remained on the cached `1.2.21` manifest after promotion observation, while immutable `1.2.23` was correct. China and overseas Den were updated serially through configuration-only restarts; both publicly read back `latestAppVersion: 1.2.23` with `1.2.23` in published inventory, and their active binary SHA-256 values matched at `82cd90a3a6ba606ff9c3cad9dece396e1b896db3c756b1daf4eee1862b7648ae`. The website was deployed from source commit `8dc6b97` to immutable root `/usr/share/nginx/jugglework-website/releases/20260923T115826Z-mac-1.2.23-8dc6b97`; both public origins verified the Mac DMG URL and retained Windows 1.2.21 URLs. Website predecessor: `/usr/share/nginx/jugglework-website/releases/20260921T055432Z-mac-1.2.22-fddb9ec`; config backup: `/etc/nginx/backup-website-mac-1.2.23-20260923T115826Z`.

## 29. Publish notarized macOS ARM64 1.2.24

- [x] 29.1 Record explicit one-release exceptions for local real-client upgrade canary and forced Stable cache expiry/refresh/public-convergence validation, scoped only to stable macOS ARM64 `1.2.24`; preserve all signing, notarization, immutable verification, locked Stable object metadata, and serialized Den checks
- [x] 29.2 Synchronize App, Desktop, and embedded Server versions to `1.2.24`; validate release tooling and OpenSpec; commit and push `dev`, then fast-forward `main`
- [x] 29.3 Build from the exact committed source; sign, notarize, staple, Gatekeeper-verify, and fully validate ARM64 ZIP and DMG plus blockmaps and updater manifest
- [x] 29.4 Upload and verify all five immutable Qiniu objects; write and verify the Stable object under the audited cache exception without forced CDN convergence
- [x] 29.5 Update China and overseas Den serially; deploy the website Mac download link and the explicitly included ICP-filing changes

Release evidence: source commit `d08c5b4b20887725ccc309d1c0f3facf515d8601` (both `dev` and `main` before this evidence record); immutable/Stable Qiniu manifest SHA-256 `c798ce16985d070b7b3f8a52f2b06aeda48501c213c8ad07317c41420f18cd13`, size 746, ETag `FlFC5Q-m030LDyPr3bns3Q9UizDA`; ZIP SHA-256 `7ebc5577469f8d8b29c126bd29726beb7a65bf8a874438db44b6f278bb765cf1`; DMG SHA-256 `ac8b00a716038b34551d6b0eaf118faa3b8313021d29d8da57eabcfc1e12da2d`. Developer ID signed app/DMG, separate Apple-accepted app submission `47c86d76-d582-49f7-9270-896c7629062e` and DMG submission `7b60a8a9-76b2-46f6-b146-f523694944ce`, validated staples, Gatekeeper, disk image, packaged ARM64 ZIP/DMG, blockmaps and updater manifest all passed. All five immutable objects passed Qiniu metadata and public CDN byte/digest checks. Stable promotion under the `stable-1.2.24-only` exceptions verified the mutable Qiniu object and released the lock; no real installed-client canary, forced cache expiry or CDN refresh occurred. A later single public Stable GET returned the 746-byte 1.2.24 digest but had `Cache-Control: public, max-age=31536000`; that observation does not establish global cache convergence or real-client automatic-update discovery. China then overseas Den publicly advertised `latestAppVersion: 1.2.24`, with published inventory and effective allowlist including 1.2.24 after config-only restarts; server binary SHA-256 remained `82cd90a3a6ba606ff9c3cad9dece396e1b896db3c756b1daf4eee1862b7648ae`. China config backup: `/root/jugglework-server/backups/desktop-version-1.2.24-china-20260924T063642Z`; overseas: `/root/jugglework-server/backups/desktop-version-1.2.24-overseas-20260924T064036Z`. Website source commit `dfa6d645ec45fda149fac08cdca8db22283663df` deployed to `/usr/share/nginx/jugglework-website/releases/20260924T064845Z-mac-1.2.24-dfa6d64`; actual canonical domain is `jugglework.cn`, while `jugglechat.cn` and `www.jugglechat.cn` redirect there. All eight files matched public bytes through both redirecting domains and both canonical domains. Website predecessor: `/usr/share/nginx/jugglework-website/releases/20260924T-no-public-security-3d1c5ea`; Nginx config backup: `/etc/nginx/backup-website-mac-1.2.24-20260924T064845Z`.
