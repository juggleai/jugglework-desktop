...
## 6. 运行中停止按钮（2026-09-08）

- [x] 6.1 Stop 按钮改为浅色黑底白色实心方块、深色白底黑色实心方块，保留尺寸、队列角标与停止行为。
- [x] 6.2 补充配色回归测试，并运行输入框相关测试及类型检查（11 项测试、前端类型检查、OpenSpec 严格校验及 diff 检查通过）。

## 7. 会话列表运行图标（2026-09-08）

- [x] 7.1 会话行尾任务运行图标在暗色主题使用白色，亮色主题保持既有强调色，且不影响其他位置的共享加载图标。
- [x] 7.2 补充会话列表配色回归测试，并完成前端类型检查、OpenSpec 严格校验及 diff 检查。
