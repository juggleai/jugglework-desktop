...
- **The IM-push delivery path is a wake-up signal riding the renderer's existing chat connection, not `apps/server`'s own dedicated IM connection receiving delivery content** → `apps/server` never connects to JuggleIM directly; the renderer recognizes jugglework-server's `jw:automation-event-delivery` system message and forwards a "poll now" signal to `apps/server` over the existing local HTTP API, which still fetches the actual delivery through the polling endpoints. This meets the latency goal (near-immediate instead of waiting out the poll interval) without the larger, separate work of giving `apps/server` its own IM identity. One consequence: `deliveryMode`'s per-automation `强制 IM 推送`/`强制轮询` choice has no runtime effect — the wake-up applies device-wide to every event-triggered automation regardless of which mode any single one is configured with.
