...
- Markdown 文件默认以渲染预览打开，正确展示 GFM 表格与 Mermaid 图形，并可切换到源码编辑后保存。
- HTML 文件预览改为无同源权限的沙箱文档，并在解析产物内容前注入限制性 CSP，阻止访问父 Renderer、Electron bridge、持久化存储、外部网络、表单、弹窗与顶层导航；非文本 HTML 安全降级为不可预览。
...
- 新增 Mermaid 前端渲染依赖；不改动主进程 IPC，不改动服务端路由。
