...
- 【文件】面板顶部提供两个 tab：
  ...
  - **变更**：展示当前会话累计产生的文件改动（每文件状态、增删行数、逐行 diff）；会话累计改动为 0 时该 tab 置灰不可点击。
...
- 数据来源：引擎 SDK `file.list`（目录懒加载）与 `session.diff`（会话累计改动）；文件读写继续走 JuggleWork Server `/workspace/:id/files/content`。
