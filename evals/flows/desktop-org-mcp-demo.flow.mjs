 ...
 * - The connection appears in JuggleWork Connect as an org MCP item that needs
 *   Jordan's sign-in.
 * - Jordan clicks through Connect, the OS browser completes a real OAuth
 *   round trip, and the item becomes Ready in Connect.
 * - A real chat turn executes the external MCP tool through JuggleWork Cloud
...
const DEN_WEB_URL = (process.env.JUGGLEWORK_EVAL_DEN_WEB_URL ?? DEN_API_URL.replace("127.0.0.1", "localhost")).trim().replace(/\/+$/, "");
...
const WORKSPACE_PATH = `/tmp/jugglework-desktop-org-mcp-demo-${RUN_TAG}`;
...
async function ensureMember(ctx) {
  ...
  ctx.assert(MARK_VERIFIED_CMD.length > 0, "Set JUGGLEWORK_EVAL_MARK_VERIFIED_CMD to verify the member's email.");
...
async function ensureWorkspace(ctx) {
  ...
  while (Date.now() < deadline) {
    const state = await ctx.eval(`(() => {
      ...
      const hasCreateAction = !hasOnboardingStep && window.__juggleworkControl?.listActions?.().find((a) => a.id === 'workspace.create')?.disabled === false;
...
async function waitForRuntimeCloudControlMcp(ctx) {
  ...
  ctx.assert(false, `Runtime JuggleWork Cloud Control MCP config never became ready: ${JSON.stringify(last)}`);
}
    ...
    created = await ctx.eval(`(async () => {
      try {
        ...
        const response = await fetch(base + '/workspaces/local', {
          method: 'POST',
          headers,
          body: JSON.stringify({ folderPath: ${JSON.stringify(WORKSPACE_PATH)}, name: 'jugglework-desktop-org-mcp-demo', preset: 'starter' }),
        });
        const text = await response.text();
        let payload = null;
        try { payload = JSON.parse(text); } catch {}
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("Boolean((localStorage.getItem('jugglework.den.authToken') ?? '').trim())", { timeoutMs: 45_000, label: "persisted den auth token" });
        await ctx.waitFor("Boolean((localStorage.getItem('jugglework.den.activeOrgId') ?? '').trim())", { timeoutMs: 60_000, label: "active org resolved" });
        await createFreshEvalWorkspace(ctx);
      },
    },
    {
      name: "Jordan discovers the org MCP connection in JuggleWork Connect",
      run: async (ctx) => {
        ...
        await ctx.prove("Jordan sees the org MCP connection in JuggleWork Connect, with a connect action", {
          voiceover: "Jordan opens JuggleWork Connect and sees the org-shared MCP connection under Needs your sign-in, clearly marked as something they can connect with their own account.",
          assert: async () => {
          ...
          screenshot: {
            ...
            claim: "JuggleWork Connect shows the org-published MCP connection with a 'Connect your account' action.",
    ...
    {
      name: "The item becomes ready in JuggleWork Connect",
      run: async (ctx) => {
        await openConnectSettings(ctx);
        await waitForConnectReadyRow(ctx, CONNECTION_NAME);
        await ctx.prove("After the browser sign-in completes, the same org MCP item is ready in JuggleWork Connect", {
          voiceover: "The OAuth callback completed in the browser, and the desktop did not need a manual reload. Jordan's row is now ready in JuggleWork Connect as a connected MCP integration.",
          assert: async () => {
            await ctx.waitFor(
              `(() => {
                const row = [...document.querySelectorAll('[data-testid="connect-organization-row"]')]
          ...
          screenshot: {
            ...
            claim: "The org MCP connection is marked Ready in JuggleWork Connect after the browser OAuth round trip.",
    ...
    {
      name: "JuggleWork Cloud Control is ready",
      run: async (ctx) => {
        ...
        const alreadyConnected = await ctx.eval(`(() => {
          const card = [...document.querySelectorAll('button')].find((el) => el.textContent.includes('JuggleWork Cloud Control'));
        ...
        if (!alreadyConnected) {
          ...
          ctx.assert(clicked, "Could not click Connect for JuggleWork Cloud Control.");
        }
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Jordan's agent can immediately execute the newly connected org MCP tool", {
          voiceover: "The connection is not just a settings card. In a real desktop chat, the agent discovers the organization's MCP capability through JuggleWork Cloud Control and executes it with Jordan's own connected account.",
          action: async () => {
            await ctx.waitFor(
              "window.__juggleworkControl?.listActions?.().find((a) => a.id === 'session.create_task')?.disabled === false",
            ...
            const prompt = `Use the JuggleWork Cloud Control connection: call search_capabilities with query "echo", then call execute_capability with the exact match name and body {"text":"${ECHO_TEXT}"}. Reply with the exact text the tool returned.`;
            await ctx.control("composer.set_text", { text: prompt });
            await ctx.waitFor(
              "window.__juggleworkControl?.listActions?.().find((a) => a.id === 'composer.send')?.disabled === false",
          ...
          screenshot: {
            ...
            claim: "A real desktop chat executed the org MCP tool through JuggleWork Cloud Control and showed the exact result.",
