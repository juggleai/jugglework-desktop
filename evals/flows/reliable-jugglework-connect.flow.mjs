...
const FLOW_ID = "reliable-jugglework-connect";
const MCP_NAME = "jugglework";
const MCP_PATH = "/mcp/agent";
const PUBLIC_MCP_SERVER_URL = "https://work.juggle.im/jwork/api/mcp/agent";
const CLIENT_SCOPE = "mcp:read mcp:write offline_access";
const CLIENT_NAME = "JuggleWork reliable connect eval client";
const OPENCODE_BIN = process.env.JUGGLEWORK_EVAL_OPENCODE_BIN?.trim() || "opencode";
...
const CODEX_COMMAND = `codex mcp add jugglework --url ${PUBLIC_MCP_SERVER_URL}`;
...
// Narration is loaded from the approved script (evals/voiceovers/reliable-jugglework-connect.md).
...
async function ensureLandingConnect(ctx) {
  const url = `${baseUrlFromEnv(ctx, "JUGGLEWORK_EVAL_LANDING_URL")}/#connect-mcp`;
  await applyDesktopViewport(ctx);
  await navigateBrowser(ctx, url, "landing JuggleWork Connect section");
  await ctx.waitFor(
    `(() => {
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>JuggleWork MCP client - ${escapeHtml(input.status)}</title>
    <style>
...
async function startLoopbackServer() {
  ...
  let details = ["The browser is authorizing JuggleWork MCP access."];
  let resolveCallback;
  ...
  const server = http.createServer((request, response) => {
    ...
    if (requestUrl.pathname === "/callback") {
      ...
      details = [
        "JuggleWork redirected back to the client loopback callback.",
...
async function prepareNativeOpenCodeEnvironment() {
  ...
  const root = await mkdtemp(path.join(os.tmpdir(), `jugglework-reliable-connect-${state.nativeRunId}-`));
  ...
  state.nativeEnv = {
    ...
    JUGGLEWORK_EVAL_BROWSER_CAPTURE_ENDPOINT: state.nativeBrowserCaptureEndpoint,
    NO_COLOR: "1",
  };
...
const endpoint = process.env.JUGGLEWORK_EVAL_BROWSER_CAPTURE_ENDPOINT || "";
const launcher = process.env.JUGGLEWORK_EVAL_BROWSER_LAUNCHER || "BROWSER";
...
JUGGLEWORK_EVAL_BROWSER_LAUNCHER=${JSON.stringify(launcherName)} exec ${JSON.stringify(state.nativeBrowserCaptureScript)} "$@"
`;
  await Promise.all([
...
function summarizeCommandResult(result) {
  ...
  return {
    ...
    mentionsJuggleWork: /\bjugglework\b/i.test(combined),
...
function commandLooksAuthenticated(result) {
  ...
  return result.exitCode === 0 && summary.mentionsJuggleWork && summary.mentionsAuthenticated && summary.mentionsOAuth && !summary.containsUnredactedToken;
}

function commandLooksConnected(result) {
  const summary = summarizeCommandResult(result);
  return result.exitCode === 0 && summary.mentionsJuggleWork && summary.mentionsConnected && !summary.containsUnredactedToken;
}
...
async function submitSignIn(ctx) {
  ...
  })()`, { timeoutMs: 60_000, label: "JuggleWork OAuth sign-in or consent" });
...
async function screenshotWithTemporarilyQuerylessUrl(ctx, name, options) {
  ...
  const sanitizedHref = await ctx.eval(`(() => {
    window.__juggleworkReliableConnectRawHref = location.href;
    const url = new URL(location.href);
    history.replaceState(history.state, document.title, url.origin + url.pathname);
    return location.href;
  })()`);
  ...
  try {
    ...
    await ctx.eval(`(() => {
      const rawHref = window.__juggleworkReliableConnectRawHref;
      if (typeof rawHref === "string" && rawHref.length > 0) {
        history.replaceState(history.state, document.title, rawHref);
      }
      delete window.__juggleworkReliableConnectRawHref;
      return true;
...
function renderProofPage(proof) {
  ...
  const commandRows = proof.commands.map((command) => `<tr>
    ...
    <td>exit ${escapeHtml(String(command.exitCode))}; stdout <code>${escapeHtml(command.stdoutHash)}</code>; stderr <code>${escapeHtml(command.stderrHash)}</code>; jugglework=${escapeHtml(String(command.mentionsJuggleWork))}; oauth=${escapeHtml(String(command.mentionsOAuth))}; connected=${escapeHtml(String(command.mentionsConnected))}; authenticated=${escapeHtml(String(command.mentionsAuthenticated))}</td>
  </tr>`).join("\n");
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Reliable JuggleWork Connect proof</title>
    <style>
  ...
  <body>
    <main>
      ...
      <h1>Reliable JuggleWork Connect proof</h1>
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>JuggleWork Connect error matrix</title>
    <style>
  ...
  <body>
    <main>
      <h1>JuggleWork Connect error matrix</h1>
...
async function runMysql(ctx, sql) {
  ...
  const database = ctx.env.JUGGLEWORK_EVAL_DEN_MYSQL_DATABASE.trim();
  const password = process.env.JUGGLEWORK_EVAL_DEN_MYSQL_ROOT_PASSWORD?.trim() || "password";
  try {
...
async function cleanupEvalRateLimits(ctx, phase) {
  ctx.assert(
    ctx.env.JUGGLEWORK_EVAL_ISOLATED_DATABASE.trim() === "1",
    "Rate-limit cleanup requires JUGGLEWORK_EVAL_ISOLATED_DATABASE=1 and must never run against a shared database.",
  );
...
async function generateLiveRateLimit(ctx) {
  const maxAttempts = Number(process.env.JUGGLEWORK_EVAL_OAUTH_RATE_LIMIT_MAX_ATTEMPTS?.trim() || "80");
  const tokenEndpoint = readString(state.authorizationServerMetadata, "token_endpoint");
...
async function navigateDocsPage(ctx) {
  const url = `${baseUrlFromEnv(ctx, "JUGGLEWORK_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp`;
  await navigateBrowser(ctx, url, "JuggleWork Connect docs page");
  await ctx.waitFor(
    `(() => {
      const root = document.querySelector('article, main') || document.body;
      const text = root.innerText || "";
      return text.includes(${JSON.stringify(PUBLIC_MCP_SERVER_URL)}) && text.includes('JuggleWork Connect MCP');
    })()`,
    { timeoutMs: 30_000, label: "JuggleWork Connect docs content" },
  );
  return url;
}
...
export default {
  id: FLOW_ID,
  title: "Reliable JuggleWork Connect is proven from landing page to OAuth, MCP tools, errors, and docs",
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: [
    "JUGGLEWORK_EVAL_LANDING_URL",
    "JUGGLEWORK_EVAL_DOCS_URL",
    "JUGGLEWORK_EVAL_DEN_API_URL",
    "JUGGLEWORK_EVAL_DEN_WEB_URL",
    "JUGGLEWORK_EVAL_DEN_MYSQL_CONTAINER",
    "JUGGLEWORK_EVAL_DEN_MYSQL_DATABASE",
    "JUGGLEWORK_EVAL_ISOLATED_DATABASE",
  ],
  steps: [
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("JuggleWork Connect publishes the permanent MCP server URL and labels only native-proven clients as verified.", {
          voiceover: vo[0],
          action: async () => {
          ...
          assert: async () => {
            ...
            recordAssertion(
              ...
              opencode.includes('"type": "remote"')
                ...
                && opencode.indexOf("opencode mcp logout jugglework") >= 0
                && opencode.indexOf(OPENCODE_AUTH_COMMAND, opencode.indexOf("opencode mcp logout jugglework")) > opencode.indexOf("opencode mcp logout jugglework"),
            ...
            recordAssertion(
              ...
              codex.includes(CODEX_COMMAND)
                ...
                && codex.indexOf("codex mcp logout jugglework") >= 0
                && codex.indexOf(CODEX_LOGIN_COMMAND, codex.indexOf("codex mcp logout jugglework")) > codex.indexOf("codex mcp logout jugglework"),
          ...
          await ctx.prove("The browser OAuth journey signs in, selects an organization, shows consent, and returns code plus state only.", {
            ...
            action: async () => {
              ...
              const consentScreenshotUrl = await screenshotWithTemporarilyQuerylessUrl(ctx, "frame-3-consent-before-authorize", {
                claim: "JuggleWork shows the selected organization on the MCP consent screen before authorization.",
                voiceover: vo[2],
    ...
    },
    {
      ...
      run: async (ctx) => {
        try {
          await ctx.prove("Public OAuth uses JWT access tokens plus rotating opaque refresh tokens while the verified client searches and executes JuggleWork capabilities for the selected org.", {
            voiceover: vo[3],
            action: async () => {
            ...
            assert: async () => {
              ...
              recordAssertion(
                ctx,
                "Native opencode mcp auth list and mcp list exit successfully and report authenticated connected OAuth for jugglework from real command output",
    ...
    },
    {
      ...
      run: async (ctx) => {
        try {
          await ctx.prove("JuggleWork Connect returns standards-compliant, traceable error responses for the failure modes clients hit.", {
            voiceover: vo[4],
            action: async () => {
            ...
            assert: async () => {
              const actual = await ctx.eval(`(() => {
                ...
                return {
                  ...
                  hasOpenCodeReconnect: bodyText.includes('opencode mcp logout jugglework') && bodyText.includes(${JSON.stringify(OPENCODE_AUTH_COMMAND)}),
                  ...
                  hasCodexReconnect: bodyText.includes('codex mcp logout jugglework') && bodyText.includes(${JSON.stringify(CODEX_LOGIN_COMMAND)}),
                  rows,
                  ...
                  hasInternalProxyWarning: bodyText.includes('work.juggle.im/api/den') && bodyText.includes('internal same-origin desktop proxy') && bodyText.includes('Do not paste it into external MCP clients'),
