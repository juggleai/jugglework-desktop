...
exit 1
`);
  const preview = await execFileAsync("daytona", ["preview-url", ctx.env.JUGGLEWORK_EVAL_DAYTONA_SANDBOX.trim(), "-p", String(ASSET_PORT)], {
    timeout: 30_000,
  });
...
async function mintInstallLink(ctx) {
  ...
  const config = await fetch(`${ctx.env.JUGGLEWORK_EVAL_DEN_API_URL.replace(/\/$/, "")}/v1/install-config?token=${encodeURIComponent(state.installToken)}`).then(async (response) => {
...
async function startRemoteInstaller(ctx) {
  const binary = ctx.env.JUGGLEWORK_EVAL_INSTALLER_BIN?.trim() || "/workspace/apps/installer/dist/jugglework-installer";
...
rm -rf "$work"
mkdir -p "$work"
cp '${binary}' "$work/jugglework-installer"
chmod +x "$work/jugglework-installer"
printf '%s' '${sidecar}' | base64 -d > "$work/jugglework-installer.json"
pkill -f '/tmp/acme-work-installer/jugglework-installer' 2>/dev/null || true
JUGGLEWORK_INSTALLER_UI=manual nohup "$work/jugglework-installer" >/tmp/acme-work-installer.log 2>&1 </dev/null &
...
sleep 3
cd /workspace
JUGGLEWORK_WORKSPACE_DIR=/workspace JUGGLEWORK_DESKTOP_BOOTSTRAP_PATH=/workspace/.jugglework-daytona/desktop-bootstrap.json JUGGLEWORK_ELECTRON_REMOTE_DEBUG_PORT=9825 DISPLAY=:99 bash .devcontainer/start-daytona-electron.sh --detach
...
set -euo pipefail
file=$(find "$HOME/.config" -name jugglework-ui-control.json -type f | head -n 1)
test -n "$file"
cat "$file"
`);
}
...
export default {
  ...
  title: "Organization display branding reaches download, setup, and desktop while JuggleWork's signed identity stays stable",
  kind: "user-facing",
  requiredEnv: [
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ensureRendererMounted(ctx);
        await ctx.waitFor("Boolean(window.__juggleworkControl)", { timeoutMs: 30_000, label: "desktop control surface" });
      ...
      run: async (ctx) => {
        await ctx.prove("An Example Corp owner saves Acme Work and a wordmark through Brand Appearance", {
          ...
          action: async () => {
            ...
            await setInputByPlaceholder(ctx, "JuggleWork", APP_NAME);
          ...
          assert: async () => {
            ...
            await panelEval(ctx, `(() => {
              const input = Array.from(document.querySelectorAll('input')).find((candidate) => candidate.placeholder === 'JuggleWork');
              input?.scrollIntoView({ block: 'center' });
      ...
      run: async (ctx) => {
        await ctx.prove("After sign-in, the desktop title and sidebar apply Acme Work live", {
          ...
          assert: async () => {
            ...
            state.updaterBefore = await ctx.eval("window.__JUGGLEWORK_ELECTRON__?.updater?.getChannel?.()", { awaitPromise: true });
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("Branding survives a fresh desktop process while app and updater identity remain JuggleWork-compatible", {
          voiceover: vo[4],
          action: async () => {
          ...
          assert: async () => {
            ...
            ctx.assert(/com\.juggleai\.jugglework(?:\.dev)?/.test(identity), `Unexpected application identity: ${identity}`);
            ctx.assert(!identity.includes("acme-work"), `Display name leaked into signed identity: ${identity}`);
            const updaterAfter = await ctx.eval("window.__JUGGLEWORK_ELECTRON__?.updater?.getChannel?.()", { awaitPromise: true });
