...
// (evals/voiceovers/jugglework-models-hidden-self-hosted.md).
// The runner fails this flow if the narration drifts from that script.
const FLOW_ID = "jugglework-models-hidden-self-hosted";
const vo = await loadVoiceoverParagraphs(FLOW_ID);

const SELF_HOSTED_BASE_URL = "https://den.internal.acme.test";
const STARTUP_DIALOG_TITLE = "Use JuggleWork Models without API keys";
const DEFAULT_ORG_SERVER_TEXT = "Using standard JuggleWork Cloud.";
const EDITOR_SELECTOR = '[contenteditable="true"][data-lexical-editor="true"]';

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

async function waitForControl(ctx) {
  await ctx.waitFor("Boolean(window.__juggleworkControl)", { timeoutMs: 90_000, label: "JuggleWork control API" });
}
...
async function dismissOnboardingOverlays(ctx) {
  const ATTRIBUTION_TITLE = "How did you hear about JuggleWork?";
...
async function ensureWorkspaceSession(ctx) {
  await waitForControl(ctx);
  const workspaceDir = ctx.env.JUGGLEWORK_EVAL_WORKSPACE_DIR?.trim() || "/workspace/hello";
...
async function resetPromoLatches(ctx, { startupShown }) {
  await ctx.eval(`(() => {
    ...
    if (${JSON.stringify(Boolean(startupShown))}) {
      localStorage.setItem("jugglework.juggleworkModelsPromo.startupShown", "1");
    } else {
      localStorage.removeItem("jugglework.juggleworkModelsPromo.startupShown");
    }
    localStorage.removeItem("jugglework.juggleworkModelsPromo.hidden");
    localStorage.removeItem("jugglework.juggleworkModelsPromo.lastShownAt");
    return true;
  })()`);
}
...
export default {
  id: FLOW_ID,
  title: "Self-hosted control planes hide every JuggleWork Models upsell surface; hosted keeps them",
  kind: "user-facing",
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Setup: session is ready on the default hosted JuggleWork Cloud control plane with promo latches reset (startup dialog latch pre-marked for the baseline phase).",
        });
      },
    },
    {
      name: "Frame 1 — hosted model picker pitches JuggleWork Models",
      run: async (ctx) => {
        await ctx.prove("On the hosted control plane the model picker shows the JuggleWork Models promo group", {
          voiceover: vo[0],
          action: async () => {
            await openModelPicker(ctx);
            await ctx.waitForText("JuggleWork hosted", { timeoutMs: 15_000 });
          },
          assert: async () => {
            await ctx.expectText("JuggleWork Models");
            await ctx.expectText("JuggleWork hosted");
          },
          screenshot: {
            name: "hosted-model-picker-promo",
            requireText: ["JuggleWork Models", "JuggleWork hosted"],
          },
        });
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("On the hosted control plane Settings > AI offers the JuggleWork Models subscription", {
          voiceover: vo[1],
          action: async () => {
          ...
          assert: async () => {
            ...
            await ctx.expectText("Hosted frontier models for JuggleWork tasks without managing provider API keys.");
          },
          screenshot: {
            name: "hosted-settings-ai-subscribe",
            requireText: ["JuggleWork Models", "Subscribe"],
    ...
    {
      name: "Frame 3 — hosted workspace setup offers Use JuggleWork Models",
      run: async (ctx) => {
        await ctx.prove("On the hosted control plane the workspace-setup provider step includes the Use JuggleWork Models option", {
          voiceover: vo[2],
          action: async () => {
          ...
          assert: async () => {
            ...
            await ctx.expectText("Use JuggleWork Models");
            await ctx.expectText("Bring your own API key");
            await ctx.expectText("Skip and use the free model");
          },
          screenshot: {
            name: "hosted-provider-step-jugglework-models",
            requireText: ["Power your first task", "Use JuggleWork Models", "Skip and use the free model"],
          },
        });
        await dismissOnboardingOverlays(ctx);
        await ensureWorkspaceSession(ctx);
    ...
    {
      name: "Frame 5 — no startup pitch and no JuggleWork Models in Settings > AI",
      run: async (ctx) => {
        await ctx.prove("Self-hosted: the startup dialog never auto-opens and Settings > AI drops every JuggleWork Models row", {
          voiceover: vo[4],
          action: async () => {
            await ensureWorkspaceSession(ctx);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Self-hosted: the model picker lists only real providers, no JuggleWork Models group", {
          voiceover: vo[5],
          action: async () => {
            await ctx.navigateHash("/");
            await ensureWorkspaceSession(ctx);
          ...
          assert: async () => {
            await ctx.expectNoText("JuggleWork hosted");
            await ctx.expectNoText("JuggleWork Models");
          },
          screenshot: {
            name: "self-hosted-model-picker-clean",
            requireText: ["All models"],
            rejectText: ["JuggleWork Models", "JuggleWork hosted"],
          },
        });
        await closeModelPicker(ctx);
      },
    ...
    {
      name: "Frame 7 — self-hosted provider step has no JuggleWork Models option",
      run: async (ctx) => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Clearing the server configuration returns to JuggleWork Cloud and the subscribe banner comes back", {
          voiceover: vo[7],
          action: async () => {
            await ctx.navigateHash("/settings/advanced");
