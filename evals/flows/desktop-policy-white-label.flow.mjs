  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        ...
        const route = await ctx.waitFor(
          "window.__juggleworkControl.snapshot().route",
...

    {
      ...
      run: async (ctx) => {
        ...
        await ctx.screenshot("baseline", {
          claim: "App renders with default JuggleWork accent and no brand logo.",
        });
      },
    },

    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__juggleworkControl.listActions().some(a => a.id === 'eval.brand_theme.apply')",
