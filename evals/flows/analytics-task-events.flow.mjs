 ...
 * (window.__jugglework.record("analytics.<event>")), so this flow asserts
  ...
  steps: [
    {
      name: "App booted",
      run: async (ctx) => {
        await ctx.waitFor("Boolean(window.__juggleworkControl) && Boolean(window.__jugglework)", {
          timeoutMs: 60_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__jugglework.events(200).some((e) => e.name === 'analytics.app_opened')",
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__juggleworkControl.listActions().some((a) => a.id === 'session.create_task')",
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.waitFor(
          "window.__jugglework.events(200).some((e) => e.name === 'analytics.task_created')",
          { timeoutMs: 30_000, label: "analytics.task_created in inspector" },
        );
        const events = await ctx.eval(
          "window.__jugglework.events(200).filter((e) => e.name.startsWith('analytics.')).map((e) => e.name)",
        );
