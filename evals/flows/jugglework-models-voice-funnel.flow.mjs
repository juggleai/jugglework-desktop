...
const EMAIL_DOMAIN = "voice-eval.jugglework.test";
const PASSWORD = "JuggleWorkVoiceEval123!";
const MOCK_INFERENCE_KEY = "ow_inf_voice_funnel";
const DEFAULT_STRIPE_WEBHOOK_SECRET = "whsec_jugglework_eval";
const DEFAULT_STRIPE_PRICE_ID = "price_jugglework_models_eval";
...
function jsonHeaders(token) {
  return {
    ...
    origin: process.env.JUGGLEWORK_EVAL_DEN_ORIGIN?.trim() || process.env.JUGGLEWORK_EVAL_DEN_API_URL?.trim()?.replace(/\/+$/, "") || "http://localhost:8790",
    ...
    data: {
      object: {
        ...
        metadata: {
          ...
          jugglework_product: "jugglework_models",
...
export default {
  id: "jugglework-models-voice-funnel",
  title: "Sign up, pay for JuggleWork Models, and start managed Voice Mode",
  spec: "evals/onboarding-welcome-flows.md#flow-28--jugglework-models-path-explains-payment-before-value",
  requiredEnv: ["JUGGLEWORK_EVAL_DEN_API_URL"],
  steps: [
    {
      name: "Start mock JuggleWork Models voice broker",
      run: async (ctx) => {
        ctx.mockBroker = await startMockBroker();
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice broker fixture is listening for authenticated JuggleWork Models session requests.",
    ...
    {
      name: "Record paid JuggleWork Models subscription",
      run: async (ctx) => {
        const secret = ctx.env.JUGGLEWORK_EVAL_STRIPE_WEBHOOK_SECRET?.trim() || DEFAULT_STRIPE_WEBHOOK_SECRET;
        const priceId = ctx.env.JUGGLEWORK_EVAL_STRIPE_INFERENCE_PRICE_ID?.trim() || DEFAULT_STRIPE_PRICE_ID;
        ...
        ctx.assert(billing.json?.billing?.stripe?.hasActiveSubscription === true, "Billing status did not show an active JuggleWork Models subscription.");
        ctx.recordEvidence({ type: "assertion", status: "passed", assertion: "The paid JuggleWork Models subscription boundary is active for the new organization.", actual: billing.json.billing.stripe.subscription });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("jugglework.den.baseUrl", ${JSON.stringify(apiBase(ctx))});
          localStorage.setItem("jugglework.den.apiBaseUrl", ${JSON.stringify(apiBase(ctx))});
          for (const key of ["jugglework.den.authToken", "jugglework.den.activeOrgId", "jugglework.den.activeOrgSlug", "jugglework.den.activeOrgName"]) localStorage.removeItem(key);
          window.location.hash = "#/settings/cloud-account";
          window.location.reload();
          return true;
        })()`);
        await ctx.waitFor("Boolean(window.__juggleworkControl)", { timeoutMs: 60_000, label: "JuggleWork control API after auth reset" });

        const handoff = await denRequest(ctx, "/v1/auth/desktop-handoff", {
          method: "POST",
          token: ctx.den.token,
          body: { desktopScheme: "jugglework" },
        });
        ctx.assert(handoff.response.ok, `Desktop handoff creation failed: ${handoff.response.status} ${handoff.text.slice(0, 300)}`);
        ctx.assert(typeof handoff.json?.juggleworkUrl === "string", "Desktop handoff did not return juggleworkUrl.");
        ...
        if (!(await ctx.hasText("Sign out"))) {
          ...
          await ctx.fill("#den-signin-link", handoff.json.juggleworkUrl);
          await ctx.clickText("Finish sign-in", { timeoutMs: 15_000 });
        }
        await ctx.waitFor(`localStorage.getItem("jugglework.den.authToken") === ${JSON.stringify(ctx.den.token)}`, {
          timeoutMs: 45_000,
        ...
        await ctx.screenshot("paid-den-account-signed-in", { claim: "Desktop is signed in to the newly paid JuggleWork Models account.", requireText: ["Sign out"] });
      },
    },
    {
      name: "Create workspace via UI",
      run: async (ctx) => {
        const workspaceDir = ctx.env.JUGGLEWORK_EVAL_WORKSPACE_DIR?.trim() || join(tmpdir(), `jugglework-models-voice-${Date.now()}`);
        ...
        await ctx.eval(`(() => {
          ...
          if (nameInput) {
            ...
            setter.call(nameInput, "JuggleWork Models Voice Eval");
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("jugglework.extension.enabled.jugglework-voice", "1");
          window.dispatchEvent(new CustomEvent("jugglework:extension-state-changed", { detail: { id: "jugglework-voice", enabled: true } }));
          return true;
        })()`);

        await ctx.expectText("JuggleWork Models", { timeoutMs: 30_000 });
        await ctx.screenshot("workspace-created-via-ui", { claim: "Workspace was created through the Add workspace UI flow.", requireText: ["JuggleWork Models"] });
      },
    },
    {
      ...
      run: async (ctx) => {
        const serverConfig = await ctx.eval(`(() => ({
          baseUrl: localStorage.getItem("jugglework.server.urlOverride") || localStorage.getItem("jugglework.server.active") || "",
          hostToken: localStorage.getItem("jugglework.server.hostToken") || "",
        }))()`);
        ...
        for (const [key, value] of [["JUGGLEWORK_API_KEY", MOCK_INFERENCE_KEY], ["JUGGLEWORK_INFERENCE_BASE_URL", ctx.mockBroker.baseUrl]]) {
          ...
          if (!savedViaUI) {
            ...
            const envResponse = await fetch(`${serverConfig.baseUrl}/env`, {
              method: "PUT",
              headers: { "x-jugglework-host-token": serverConfig.hostToken, "content-type": "application/json" },
            ...
            await ctx.waitFor("Boolean(window.__juggleworkControl)", { timeoutMs: 30_000, label: "control API after env reload" });
            await ctx.clickText("Environment", { timeoutMs: 10_000 });
            await sleep(1_000);
          }
          ctx.log(`Saved ${key}`);
        ...
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice credentials (JUGGLEWORK_API_KEY + JUGGLEWORK_INFERENCE_BASE_URL) were saved through the Settings > Environment UI.",
          actual: ["JUGGLEWORK_API_KEY", "JUGGLEWORK_INFERENCE_BASE_URL"],
        });
        await ctx.screenshot("managed-voice-env-configured-via-ui", {
          ...
          requireText: ["JUGGLEWORK_API_KEY", "JUGGLEWORK_INFERENCE_BASE_URL"],
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("window.__juggleworkControl.listActions().some((action) => action.id === 'voice.panel.open' && !action.disabled)", {
          timeoutMs: 30_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        ctx.assert(request.authorization === `Bearer ${MOCK_INFERENCE_KEY}`, "Voice Mode did not authenticate to the JuggleWork Models broker with the paid inference key.");
        ctx.assert(request.url === "/voice/realtime/session", "Voice Mode did not call the managed realtime session endpoint.");
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Voice Mode requested a managed JuggleWork Models realtime session.",
        ...
        ctx.assert(
          ...
          "Voice Mode showed a direct OpenAI API key missing error instead of using the managed JuggleWork Models path.",
        );
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
        ...
        await ctx.screenshot("managed-voice-mode-started", {
          claim: "Voice Mode started through the managed JuggleWork Models session path via UI click.",
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const statusResult = await ctx.eval(`window.__juggleworkControl.execute("voice.status")`);
        ...
        await ctx.screenshot("voice-mode-active-with-transcript", {
          claim: "Voice Mode is active through the managed JuggleWork Models path, with timeline and connection state visible.",
          requireText: ["Voice Mode"],
          rejectText: ["OpenAI API key missing"],
        });
