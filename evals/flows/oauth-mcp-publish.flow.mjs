 ...
 *   1. Signs in to JuggleWork Cloud via desktop handoff and creates a workspace.
 ...
 * seeded demo org (alex@acme.test / JuggleWorkDemo123!).
 */
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Owner is signed in to JuggleWork Cloud", {
          action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const wsPath = join(homedir(), ".jugglework", "two-electron-demo", "eval-workspace-a");
