...
const vo = await loadVoiceoverParagraphs("docs-jugglework-connect");
...
const OPENCODE_RECONNECT_LOGOUT = "opencode mcp logout jugglework";
const CODEX_ADD_COMMAND = `codex mcp add jugglework --url ${MCP_SERVER_URL}`;
const CODEX_LOGIN_COMMAND = "codex mcp login jugglework";
const CODEX_RECONNECT_LOGOUT = "codex mcp logout jugglework";
...
async function waitForInstaller(ctx) {
  await ctx.waitFor(
    ...
    { timeoutMs: 30_000, label: "JuggleWork Connect installer" },
  );
...
async function grantDocsClipboardPermissions(ctx) {
  ...
  await ctx.client.send("Browser.grantPermissions", {
    origin: new URL(baseUrl("JUGGLEWORK_EVAL_DOCS_URL")).origin,
    permissions: ["clipboardReadWrite", "clipboardSanitizedWrite"],
  });
}

export default {
  id: "docs-jugglework-connect",
  title: "Use the JuggleWork Connect installer from the docs or landing page",
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["JUGGLEWORK_EVAL_DOCS_URL", "JUGGLEWORK_EVAL_LANDING_URL"],
  steps: [
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("The JuggleWork Connect docs put the complete client installer at the top of the page.", {
          voiceover: vo[0],
          action: async () => {
            await navigate(ctx, `${baseUrl("JUGGLEWORK_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp`);
          ...
          assert: async () => {
            const actual = await ctx.eval(`(() => {
              ...
              return {
                ...
                supportRows,
                hasInternalProxy: bodyText.includes("work.juggle.im/api/den") && bodyText.includes("internal same-origin desktop proxy"),
                hasAuthServer: bodyText.includes("https://work.juggle.im/api/auth"),
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The direct OpenCode docs link opens the installer with OpenCode selected.", {
          voiceover: vo[3],
          action: async () => {
            await navigate(ctx, `${baseUrl("JUGGLEWORK_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp#connect-mcp-install-opencode`);
            await waitForInstaller(ctx);
          },
          assert: async () => {
            const actual = await ctx.eval(`(() => {
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("The landing page keeps the same JuggleWork Connect installer, including Codex and ChatGPT Desktop.", {
          voiceover: vo[4],
          action: async () => {
            await navigate(ctx, `${baseUrl("JUGGLEWORK_EVAL_LANDING_URL")}/#connect-mcp`);
            await waitForInstaller(ctx);
