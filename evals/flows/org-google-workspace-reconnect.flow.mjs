...
async function approveMockConsent(ctx) {
  const clicked = await ctx.eval(`(() => {
    const button = [...document.querySelectorAll('button')].find((entry) => (entry.textContent ?? '').trim() === 'Approve JuggleWork');
    button?.click();
    return Boolean(button);
  })()`);
  ctx.assert(clicked, "Mock OAuth consent page did not show an Approve JuggleWork button.");
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Clicking Reconnect opens consent for the newly requested Google scopes", {
          ...
          action: async () => {
            await ctx.eval(`(() => {
              window.__juggleworkEvalLastOpen = null;
              window.open = (url) => { window.__juggleworkEvalLastOpen = String(url); return null; };
              return true;
            })()`);
            ...
            await ctx.waitFor("typeof window.__juggleworkEvalLastOpen === 'string'", { timeoutMs: 30_000, label: "Reconnect authorize URL opened" });
            state.reconnectAuthorizeUrl = await ctx.eval("window.__juggleworkEvalLastOpen");
          ...
          screenshot: {
            ...
            requireText: ["Mock MCP OAuth", "Requested scopes", "https://www.googleapis.com/auth/gmail.readonly", "https://www.googleapis.com/auth/calendar.events", "Approve JuggleWork"],
