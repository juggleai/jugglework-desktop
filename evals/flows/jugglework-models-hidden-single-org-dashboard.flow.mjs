...
// (evals/voiceovers/jugglework-models-hidden-single-org-dashboard.md).
// The runner fails this flow if the narration drifts from that script.
const FLOW_ID = "jugglework-models-hidden-single-org-dashboard";
const vo = await loadVoiceoverParagraphs(FLOW_ID);

const DEMO_EMAIL = process.env.DEN_DEMO_OWNER_EMAIL ?? "alex@acme.test";
const DEMO_PASSWORD = process.env.DEN_DEMO_OWNER_PASSWORD ?? "JuggleWorkDemo123!";

export default {
  id: FLOW_ID,
  title: "Single-org (self-hosted) dashboard hides the JuggleWork Models nav and page",
  kind: "user-facing",
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The Models nav group contains no JuggleWork Models entry and lands on LLM Providers", {
          voiceover: vo[1],
          action: async () => {
            await ctx.waitFor(`(() => {
          ...
          assert: async () => {
            const nav = await ctx.eval(`(() => {
              ...
              return {
                ...
                hasJuggleWorkModels: links.some((l) => l.text.includes('JuggleWork Models')) || (nav?.innerText ?? '').includes('JuggleWork Models'),
            ...
            ctx.assert(!nav.hasJuggleWorkModels, "The navigation must not mention JuggleWork Models on a self-hosted deployment.");
      ...
      run: async (ctx) => {
        await ctx.prove("Visiting /dashboard/inference directly redirects to LLM Providers", {
          ...
          screenshot: {
            ...
            requireText: ["LLM Providers"],
            rejectText: ["Subscribe with Stripe", "JuggleWork Models"],
          },
        });
      },
    },
