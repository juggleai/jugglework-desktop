 ...
 * - den-api at JUGGLEWORK_EVAL_DEN_API_URL, den-web at JUGGLEWORK_EVAL_DEN_WEB_URL.
 * - Mock OAuth+MCP server reachable at MOCK_OAUTH_MCP_URL from den-api.
 * - Admin: JUGGLEWORK_EVAL_DEMO_EMAIL/PASSWORD (default seeded alex@acme.test).
 * - Member: JUGGLEWORK_EVAL_MEMBER_EMAIL/PASSWORD (default
 *   jordan.demo@acme.test / JuggleWorkDemo123!). If the member can't sign in,
 ...
 *   JUGGLEWORK_EVAL_MARK_VERIFIED_CMD — a shell template with {email}, e.g.
      ...
      run: async (ctx) => {
        ...
        if (!state.memberSession) {
          ...
          ctx.assert(
            ...
            "Member email must be verified to accept the invitation; set JUGGLEWORK_EVAL_MARK_VERIFIED_CMD (shell template with {email}).",
          );
