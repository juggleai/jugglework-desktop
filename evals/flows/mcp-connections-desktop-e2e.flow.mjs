 ...
 * happens in the JuggleWork DESKTOP APP, from the member's point of view:
 *
 ...
 *
 *   1. Jordan signs the DESKTOP APP into JuggleWork Cloud (real handoff
 *      grant exchange against the local Den).
 *   2. The app auto-configures "JuggleWork Cloud Control" — the /mcp/agent
 ...
 * - den-api at JUGGLEWORK_EVAL_DEN_API_URL with the seeded demo org.
 ...
 *   automatically with JUGGLEWORK_EVAL_MARK_VERIFIED_CMD if missing).
...
const WORKSPACE_PATH = "/tmp/jugglework-mcp-desktop-e2e";
...
export default {
  ...
  steps: [
    ...
    {
      name: "Member signs the desktop app into JuggleWork Cloud (real handoff grant)",
      run: async (ctx) => {
    ...
    {
      name: "JuggleWork Cloud Control auto-configures with the member's token",
      run: async (ctx) => {
        ...
        await ctx.waitFor(
          ...
          { timeoutMs: 120_000, label: "jugglework.den.mcp.sync marker" },
        );
        ctx.log(`marker: ${await ctx.eval("localStorage.getItem('jugglework.den.mcp.sync')")}`);
        await ctx.prove("JuggleWork Cloud Control is configured in the member's app", {
          assert: async () => {
            await ctx.expectText("JuggleWork Cloud Control", { timeoutMs: 30_000 });
          },
          screenshot: {
            name: "desktop-cloud-control-configured",
            claim: "The member's desktop app auto-configured the JuggleWork Cloud Control connection after cloud sign-in.",
