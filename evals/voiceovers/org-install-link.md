...
1. Alex wants the whole team on JuggleWork, so from the team page he copies Acme's install link — one button, and it's ready to paste into Slack, an email, anywhere.
...
3. They run it, and before touching anything it says exactly what it's about to do: set up JuggleWork for Acme Robotics, on Acme's workspace — one click to continue.

4. The installer fetches the right app version and writes Acme's configuration, so when JuggleWork opens for the first time it isn't an empty app asking them to pick a folder — it's Acme's app, asking them to sign in.

5. One click on Sign in with JuggleWork Cloud, a browser flash, and they're standing in Acme's workspace — nobody typed a server URL, ever.
