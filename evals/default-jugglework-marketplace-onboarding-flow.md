# Default JuggleWork Marketplace onboarding flow
...
Marketplace containing built-in JuggleWork extension capabilities.

## Acceptance target

- Den download/sign-in copy does not claim that Den creates or loads a desktop workspace.
- Den introduces the built-in JuggleWork Marketplace and explains it appears after desktop sign-in.
- A signed-in desktop user can open Settings -> Marketplace and see `JuggleWork Marketplace` from Den.
- `JuggleWork Marketplace` includes built-in extension entries such as `JuggleWork Browser`, `Computer Use`, `OpenAI Image Gen`, `Google Workspace`, and `Ollama`.
- Built-in JuggleWork entries are rendered as cloud Marketplace entries with a `Built-in` status, not as locally injected desktop Marketplace rows.
- Signed-out desktop users can still use JuggleWork, but Marketplace nudges sign-in for built-in extensions and organization marketplaces.
...
8. Validate dev auth before UI recording: direct Den API sign-in with the seeded user should return `200` when `JUGGLEWORK_DEV_MODE=1`.
...
- Page includes `built-in JuggleWork Marketplace` or `built-in Marketplace`.
...
**Goal:** JuggleWork remains usable without account, while Marketplace clearly asks
for Cloud sign-in.
...
- The notice says the user can use JuggleWork without an account.
...
**Goal:** Desktop sign-in causes Den to provision and return the default JuggleWork
Marketplace for any user/org.

Steps:

1. Sign Electron into JuggleWork Cloud using the Daytona Den handoff flow.
2. Create or open a local workspace.
3. Open Settings -> Marketplace.
4. Click `Refresh` if the Marketplace list has not loaded yet.
5. Open `Filters` and inspect marketplace options.
6. Search for `JuggleWork Browser`.
7. Open the `JuggleWork Browser` card.

Expected outcome:

- `JuggleWork Marketplace` appears as a marketplace option.
- `JuggleWork Browser` appears as a Marketplace card from Den.
- The card shows `Built-in` or an equivalent built-in/ready status.
- The detail modal shows JuggleWork Browser setup/resource details from the Den extension manifest.
- The detail modal does not offer `Add` or `Remove` for built-in JuggleWork entries.
...
2. Find `JuggleWork Marketplace`.
3. Call `GET /v1/marketplaces/:id/resolved`.

Expected outcome:

- The API returns `JuggleWork Marketplace`.
- Resolved plugins include the built-in JuggleWork entries.
- Each built-in plugin has `extension.sourceFormat = jugglework-builtin`.
- `JuggleWork Browser` has an extension manifest with the `opencode-chrome-devtools` resource.
...
2. Den explains in plain language: Marketplaces contain plugins; JuggleWork Marketplace is included; org marketplaces appear after desktop sign-in.
...
6. Marketplace is pre-populated without manual refresh, with `JuggleWork Marketplace` grouped separately from org marketplaces.
...
stores and returns JuggleWork extension manifests; the desktop still owns local
