...
#### One-line summary
Gets desktop users to the right download path faster and lets `jugglework-orchestrator` recover when npm skips its platform binary.
...
- `jugglework-orchestrator` postinstall now downloads the matching release binary when optional platform packages are missing.
...
#### Title
Local workspaces move under JuggleWork server control

#### One-line summary
Shifts local workspace ownership into JuggleWork server so creation, reconnects, config writes, and starter bootstrap stay aligned across the app.

#### Main changes
- Local workspace create, rename, delete, config writes, and reload events now go through JuggleWork server first.
...
#### Major bug fix details
- Fixed local workspace reconnect and onboarding inconsistencies by moving workspace ownership into JuggleWork server.
...
#### Title
Team template sharing reaches JuggleWork Cloud
...
#### Release importance
Major release: adds substantial new Cloud collaboration and organization-management workflows that materially change how teams use JuggleWork.
...
#### Major improvement details
- Added Cloud team template sharing flows in the JuggleWork app.
...
#### Title
JuggleWork resumes where you left off
...
- Moved JuggleWork tokens off CLI args and logs, stopped auto-creating Welcome, and fixed collapsed sidebar session lists.
...
- Fixed bundle fetch routing so publish and fetch traffic stays pinned to the configured JuggleWork publisher.
...
#### Main changes
- Added a JuggleWork Cloud skills catalog to the Skills page, with install and share-to-team flows.
...
#### Major improvement details
- Added the JuggleWork Cloud team skills catalog on the app Skills page.
